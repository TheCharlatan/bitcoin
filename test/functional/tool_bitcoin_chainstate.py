...
class BitcoinChainstateTest(BitcoinTestFramework):
    ...
    def add_block(self, datadir, input, expected_stderr):
        ...
        stdout, stderr = proc.communicate(input=input, timeout=5)
    ...
    def run_test(self):
        ...
        # This might trigger a warning that there is not enough disk space, since this node runs on mainnet. Ignore it.
        node.stop_node(check_stderr=False)
        ...
        self.add_block(datadir, block_one + "\n", "Block has not yet been rejected")
        self.add_block(datadir, block_one + "\n", "duplicate")
        self.add_block(datadir, "00" + "\n", "Block decode failed")
        self.add_block(datadir, "\n", "Empty line found")
