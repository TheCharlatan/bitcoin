...
class BitcoinChainstateTest(BitcoinTestFramework):
    ...
    def add_block(self, binary_args, input, expected_stderr):
    ...
    def run_test(self):
        node = self.nodes[0]
        binary_args = node.binaries.chainstate_argv() + [node.cli.datadir]
        bitcoind_args = node.binaries.util_argv() + [node.cli.datadir]
        node.stop_node()

        self.log.info(f"Testing bitcoin-chainstate: {binary_args}")
        self.log.info(f"Testing bitcoind: {bitcoind_args}")
        ...
        self.add_block(binary_args, block_one, "Block has not yet been rejected")
        self.add_block(binary_args, block_one, "duplicate")
        self.add_block(binary_args, "00", "Block decode failed")
        self.add_block(binary_args, "", "Empty line found")
