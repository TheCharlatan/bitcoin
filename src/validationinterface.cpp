...
public:
    ...
    // our own queue here :(
    SerialTaskRunner m_serial_task_runner;

    explicit ValidationSignalsImpl(CScheduler& scheduler LIFETIMEBOUND) : m_serial_task_runner(scheduler) {}
...
{
    m_internals->m_serial_task_runner.clear();
}

size_t ValidationSignals::size()
{
    return m_internals->m_serial_task_runner.size();
}
...
{
    m_internals->m_serial_task_runner.insert(std::move(func));
}
...
#define ENQUEUE_AND_LOG_EVENT(event, fmt, name, ...)           \
    do {                                                       \
        ...
        m_internals->m_serial_task_runner.insert([=] { \
