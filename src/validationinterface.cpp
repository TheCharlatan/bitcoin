...
ValidationSignals::ValidationSignals(std::unique_ptr<util::TaskRunnerInterface> task_runner)
    : m_internals{std::make_unique<ValidationSignalsImpl>()},
      m_serial_task_runner{std::move(Assert(task_runner))} {}

ValidationSignals::~ValidationSignals() {}

void ValidationSignals::FlushBackgroundCallbacks()
{
    m_serial_task_runner->clear();
}

size_t ValidationSignals::size()
{
    return m_serial_task_runner->size();
}
...
{
    m_serial_task_runner->insert(std::move(func));
}
