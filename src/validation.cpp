...
PackageMempoolAcceptResult MemPoolAccept::AcceptPackage(const Package& package, ATMPArgs& args)
{
    ...
    for (const auto& tx : package) {
        ...
        if (m_pool.exists(GenTxid::Wtxid(wtxid))) {
            ...
            const auto& entry{Assert(m_pool.GetEntry(txid)).value()};
            results_final.emplace(wtxid, MempoolAcceptResult::MempoolTx(entry.get().GetTxSize(), entry.get().GetFee()));
            ...
            results_final.emplace(wtxid, MempoolAcceptResult::MempoolTxDifferentWitness(entry.get().GetTx().GetWitnessHash()));
        } else {
