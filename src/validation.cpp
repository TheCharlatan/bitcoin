...
bool AbortNode(BlockValidationState& state, const std::string& strMessage, const std::function<void(const bilingual_str& user_message)>& init_error, const bilingual_str& userMessage)
{
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (!CheckBlock(block, state, params.GetConsensus(), !fJustCheck, !fJustCheck)) {
        if (state.GetResult() == BlockValidationResult::BLOCK_MUTATED) {
            ...
            // problems.
            return AbortNode(state, "Corrupt block found indicating potential hardware failure; shutting down", m_chainman.GetInitErrorCb());
        }
    ...
    int nManualPruneHeight)
{
    ...
    try {
    {
        ...
        if (fDoFullFlush || fPeriodicWrite) {
            ...
            if (!CheckDiskSpace(gArgs.GetBlocksDirPath())) {
                return AbortNode(state, "Disk space is too low!", m_chainman.GetInitErrorCb(), _("Disk space is too low!"));
            }
            {
            ...
            // Then update all block file information (which may refer to block and undo files).
            {
                ...
                if (!m_blockman.WriteBlockIndexDB()) {
                    return AbortNode(state, "Failed to write to block index database", m_chainman.GetInitErrorCb());
                }
            }
        ...
        if (fDoFullFlush && !CoinsTip().GetBestBlock().IsNull()) {
            ...
            if (!CoinsTip().Flush())
                return AbortNode(state, "Failed to write to coin database", m_chainman.GetInitErrorCb());
            m_last_flush = nNow;
    ...
    if (full_flush_completed) {
        ...
        return AbortNode(state, std::string("System error while flushing: ") + e.what(), m_chainman.GetInitErrorCb());
    }
    return true;
}
...
bool Chainstate::ConnectTip(BlockValidationState& state, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions& disconnectpool)
{
    ...
    if (!pblock) {
        ...
        if (!m_blockman.ReadBlockFromDisk(*pblockNew, *pindexNew)) {
            return AbortNode(state, "Failed to read block", m_chainman.GetInitErrorCb());
        }
    ...
    if (this != &m_chainman.ActiveChainstate()) {
        ...
        m_chainman.MaybeCompleteSnapshotValidation(
            [&init_error = m_chainman.GetInitErrorCb()](bilingual_str msg) {
...
bool Chainstate::ActivateBestChainStep(BlockValidationState& state, CBlockIndex* pindexMostWork, const std::shared_ptr<const CBlock>& pblock, bool& fInvalidFound, ConnectTrace& connectTrace)
{
    ...
    while (m_chain.Tip() && m_chain.Tip() != pindexFork) {
        if (!DisconnectTip(state, &disconnectpool)) {
            ...
            AbortNode(state, "Failed to disconnect block; see debug.log for details", m_chainman.GetInitErrorCb());
            return false;
        }
...
static bool NotifyHeaderTip(Chainstate& chainstate) LOCKS_EXCLUDED(cs_main) {
    ...
    if (fNotify) {
        chainstate.m_chainman.NotifyHeaderTip(GetSynchronizationState(fInitialBlockDownload), pindexHeader->nHeight, pindexHeader->nTime, false);
    }
    return fNotify;
}
...
void ChainstateManager::NotifyHeaderTip(SynchronizationState state, int64_t height, int64_t timestamp, bool presync) const
{
    return m_options.notification_callbacks.notify_header_tip(state, height, timestamp, presync);
}

void ChainstateManager::ShowProgress(const std::string& title, int nProgress, bool resume_possible) const
{
    return m_options.notification_callbacks.show_progress(title, nProgress, resume_possible);
}
...
const std::function<void(const bilingual_str&)>& ChainstateManager::GetInitErrorCb() const
{
    return m_options.notification_callbacks.init_error;
}
...
void ChainstateManager::ReportHeadersPresync(const arith_uint256& work, int64_t height, int64_t timestamp)
{
    ...
    NotifyHeaderTip(GetSynchronizationState(initial_download), height, timestamp, /*presync=*/true);
    if (initial_download) {
...
bool Chainstate::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, BlockValidationState& state, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock, bool min_pow_checked)
{
    ...
    try {
        ...
        return AbortNode(state, std::string("System error: ") + e.what(), m_chainman.GetInitErrorCb());
    }
    ...
    std::multimap<uint256, FlatFilePos>* blocks_with_unknown_parent)
{
    ...
    try {
        ...
        AbortNode(std::string("System error: ") + e.what(), m_chainman.GetInitErrorCb());
    }
        ...
        if (auto snapshot_datadir = node::FindSnapshotChainstateDir()) {
            ...
            if (!removed) {
                AbortNode(strprintf("Failed to remove snapshot chainstate dir (%s). "
                          ...
                          m_options.notification_callbacks.init_error);
            }
        }
        return false;
    }
      ...
      m_blockman{std::move(blockman_options)}
{
    ...
    assert(m_options.notification_callbacks.init_error);
}
...
bool ChainstateManager::ValidatedSnapshotCleanup()
{
    ...
    auto rename_failed_abort = [&init_error = GetInitErrorCb()](
                                   fs::path p_old,
                                   fs::path p_new,
