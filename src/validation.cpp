...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (!Consensus::CheckTxInputs(tx, state, std::span<std::reference_wrapper<const Coin>>{coins}, m_active_chainstate.m_chain.Height() + 1, ws.m_base_fees)) {
...
bool Chainstate::ConnectBlock(const CBlock& block, const uint256& block_hash, BlockValidationState& state, CBlockIndex* pindex,
                               CBlockUndo& blockundo, bool fJustCheck)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase())
        {
            ...
            if (!Consensus::CheckTxInputs(tx, tx_state, std::span<const Coin>{spent_coins}, pindex->nHeight, txfee)) {
...
bool Chainstate::ConnectTip(BlockValidationState& state, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions& disconnectpool)
{
             ...
             Ticks<MillisecondsDouble>(time_2 - time_1));
    {
        ...
        bool rv = ConnectBlock(blockConnecting, block_hash, state, pindexNew, blockundo);
        if (m_chainman.m_options.signals) {
            m_chainman.m_options.signals->BlockChecked(blockConnecting, state);
        }
        if (!rv) {
                       ...
                       bool fCheckMerkleRoot)
{
    ...
    if (!chainstate.ConnectBlock(block, block_hash, state, &indexDummy, blockundo, true)) {
        return false;
    }
    ...
    int nCheckLevel, int nCheckDepth)
{
    ...
    if (nCheckLevel >= 4 && !skipped_l3_checks) {
        while (pindex != chainstate.m_chain.Tip()) {
            ...
            if (!chainstate.ConnectBlock(block, block_hash, state, pindex, blockundo)) {
