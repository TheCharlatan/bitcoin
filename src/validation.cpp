...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (!ancestors) return state.Invalid(TxValidationResult::TX_MEMPOOL_POLICY, "too-long-mempool-chain", util::ErrorString(ancestors).original);
...
bool MemPoolAccept::ClusterSizeChecks(Workspace& ws)
{
    ...
    const CTxMemPool::setEntryRefs& ancestors = ws.m_ancestors;
    TxValidationState& state = ws.m_state;
...
bool MemPoolAccept::ReplacementChecks(Workspace& ws)
{
    ...
    CTxMemPoolEntry::Parents parents;
    for (const CTxMemPoolEntry& ancestor : ws.m_ancestors) parents.insert(ancestor);
