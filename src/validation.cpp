    ...
    int nManualPruneHeight)
{
    ...
    try {
    {
        ...
        if (m_blockman.IsPruneMode() && (m_blockman.m_check_for_pruning || nManualPruneHeight > 0) && !m_chainman.m_blockman.m_reindex) {
...
static SynchronizationState GetSynchronizationState(bool init, std::atomic<bool>& reindex)
{
    ...
    if (reindex) return SynchronizationState::INIT_REINDEX;
...
static bool NotifyHeaderTip(ChainstateManager& chainman) LOCKS_EXCLUDED(cs_main)
{
    ...
    if (fNotify) {
        chainman.GetNotifications().headerTip(GetSynchronizationState(fInitialBlockDownload, chainman.m_blockman.m_reindex), pindexHeader->nHeight, pindexHeader->nTime, false);
    }
    return fNotify;
}
    ...
    do {
...

        {
            ...
            if (this == &m_chainman.ActiveChainstate() && pindexFork != pindexNewTip) {
                ...
                if (kernel::IsInterrupted(m_chainman.GetNotifications().blockTip(GetSynchronizationState(still_in_ibd, m_chainman.m_blockman.m_reindex), *pindexNewTip))) {
...
bool Chainstate::InvalidateBlock(BlockValidationState& state, CBlockIndex* pindex)
{
    ...
    if (pindex_was_in_chain) {
        ...
        // changes.
        (void)m_chainman.GetNotifications().blockTip(GetSynchronizationState(m_chainman.IsInitialBlockDownload(), m_chainman.m_blockman.m_reindex), *to_mark_failed->pprev);
    }
    return true;
}
...
void ChainstateManager::ReportHeadersPresync(const arith_uint256& work, int64_t height, int64_t timestamp)
{
    ...
    GetNotifications().headerTip(GetSynchronizationState(initial_download, m_blockman.m_reindex), height, timestamp, /*presync=*/true);
    if (initial_download) {
...
bool ChainstateManager::LoadBlockIndex()
{
    ...
    bool needs_init = m_blockman.m_reindex;
    if (!m_blockman.m_reindex) {
    ...
    if (needs_init) {
        ...
        // LoadBlockIndexDB may have set m_reindex if we shut down
        // mid-reindex previously, we don't check m_reindex and
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                if (m_blockman.IsPruneMode() && !m_blockman.m_reindex && pblock) {
