                                  ...
                                  std::map<uint256, MempoolAcceptResult>& results)
{
    ...
    for (Workspace& ws : workspaces) {
        ...
        m_signals.TransactionAddedToMempool(ws.m_ptx, m_pool.GetAndIncrementSequence());
    }
...
MempoolAcceptResult MemPoolAccept::AcceptSingleTransaction(const CTransactionRef& ptx, ATMPArgs& args)
{
    ...
    m_signals.TransactionAddedToMempool(ptx, m_pool.GetAndIncrementSequence());
    ...
    EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    MempoolAcceptResult result = MemPoolAccept(pool, active_chainstate, active_chainstate.m_chainman.m_signals).AcceptSingleTransaction(tx, args);
                                                   ...
                                                   const Package& package, bool test_accept)
{
    ...
    auto result = [&]() EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
        ...
        if (test_accept) {
            ...
            return MemPoolAccept(pool, active_chainstate, active_chainstate.m_chainman.m_signals).AcceptMultipleTransactions(package, args);
        } else {
            auto args = MemPoolAccept::ATMPArgs::PackageChildWithParents(chainparams, GetTime(), coins_to_uncache);
            return MemPoolAccept(pool, active_chainstate, active_chainstate.m_chainman.m_signals).AcceptPackage(package, args);
        }
    }();
    ...
    int nManualPruneHeight)
{
    ...
    if (full_flush_completed) {
        ...
        m_chainman.m_signals.ChainStateFlushed(this->GetRole(), m_chain.GetLocator());
    }
...
bool Chainstate::DisconnectTip(BlockValidationState& state, DisconnectedBlockTransactions* disconnectpool)
{
    ...
    m_chainman.m_signals.BlockDisconnected(pblock, pindexDelete);
    return true;
}
...
bool Chainstate::ConnectTip(BlockValidationState& state, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions& disconnectpool)
{
             ...
             Ticks<MillisecondsDouble>(time_2 - time_1));
    {
        ...
        m_chainman.m_signals.BlockChecked(blockConnecting, state);
        if (!rv) {
            if (state.IsInvalid())
...
bool Chainstate::ActivateBestChain(BlockValidationState& state, std::shared_ptr<const CBlock> pblock)
{
    ...
    do {
        ...
        LimitValidationInterfaceQueue(m_chainman.m_signals);

        {
            ...
            do {
                ...
                for (const PerBlockConnectTrace& trace : connectTrace.GetBlocksConnected()) {
                    ...
                    m_chainman.m_signals.BlockConnected(this->GetRole(), trace.pblock, trace.pindex);
                }
            ...
            if (this == &m_chainman.ActiveChainstate() && pindexFork != pindexNewTip) {
                ...
                m_chainman.m_signals.UpdatedBlockTip(pindexNewTip, pindexFork, still_in_ibd);
