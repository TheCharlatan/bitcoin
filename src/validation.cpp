...
Chainstate::Chainstate(
    ...
    : m_mempool(mempool),
      m_validity(from_snapshot_blockhash ? ChainValidity::ASSUMED_VALID : ChainValidity::VALIDATED),
      m_blockman(blockman),
      m_chainman(chainman),
...
void Chainstate::SetTargetBlockHash(uint256 block_hash) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
...
bool Chainstate::ActivateBestChain(BlockValidationState& state, std::shared_ptr<const CBlock> pblock)
{
    ...
    do {
        ...
        if (WITH_LOCK(::cs_main, return ReachedTarget())) {
...
std::vector<Chainstate*> ChainstateManager::GetAll()
{
    ...
    for (Chainstate* cs : {m_ibd_chainstate.get(), m_snapshot_chainstate.get()}) {
        if (cs && cs->Validity() != ChainValidity::INVALID && !cs->m_target_utxohash) out.push_back(cs);
    }
...
SnapshotCompletionResult ChainstateManager::MaybeCompleteSnapshotValidation(Chainstate& validated_chainstate, Chainstate& from_snapshot_chainstate)
{
    ...
    auto handle_invalid_snapshot = [&]() EXCLUSIVE_LOCKS_REQUIRED(::cs_main) {
        bilingual_str user_error = strprintf(_(
            ...
            PACKAGE_NAME, from_snapshot_chainstate.m_chain.Height(),
            validated_chainstate.m_chain.Height(),
            validated_chainstate.m_chain.Height(), PACKAGE_BUGREPORT
        );
...
void ChainstateManager::MaybeRebalanceCaches()
{
    ...
    if (!historical_chainstate && !current_chainstate.m_from_snapshot_blockhash) {
        ...
        current_chainstate.ResizeCoinsCaches(m_total_coinstip_cache, m_total_coinsdb_cache);
    } else if (!historical_chainstate) {
        ...
        if (IsInitialBlockDownload()) {
            historical_chainstate->ResizeCoinsCaches(
                m_total_coinstip_cache * 0.05, m_total_coinsdb_cache * 0.05);
            current_chainstate.ResizeCoinsCaches(
...
util::Result<void> Chainstate::InvalidateCoinsDBOnDisk()
{
    ...
    try {
        ...
        LogPrintf("%s: error renaming file '%s' -> '%s': %s\n",
                __func__, db_path_str, invalid_path_str, e.what());
...
bool ChainstateManager::ValidatedSnapshotCleanup(Chainstate& validated_chainstate, Chainstate& from_snapshot_chainstate)
{
    ...
    const fs::path validated_path{validated_chainstate.StoragePath()};
    const fs::path from_snapshot_path{from_snapshot_chainstate.StoragePath()};
    ...
    LogPrintf("[snapshot] moving snapshot chainstate (%s) to "
              ...
              fs::PathToString(from_snapshot_path), fs::PathToString(validated_path));

    try {
        fs::rename(from_snapshot_path, validated_path);
    } catch (const fs::filesystem_error& e) {
        rename_failed_abort(from_snapshot_path, validated_path, e);
        throw;
    }
...
std::pair<int, int> ChainstateManager::GetPruneRange(const Chainstate& chainstate, int last_height_can_prune)
{
    ...
    if (chainstate.m_from_snapshot_blockhash && chainstate.Validity() != ChainValidity::VALIDATED) {
        // Only prune blocks after the snapshot if this is a snapshot chain that
        // has not been fully validated yet. The earlier blocks need to be kept
