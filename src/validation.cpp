    ...
    bool fAddToMempool)
{
    ...
    auto it = disconnectpool.queuedTx->impl.get<insertion_order>().rbegin();
    while (it != disconnectpool.queuedTx->impl.get<insertion_order>().rend()) {
    ...
    const auto filter_final_and_mature = [this](txiter& it)
        ...
        if (TestLockPointValidity(m_chain, lp)) {
            ...
            if (new_lock_points.has_value() && CheckSequenceLocksAtTip(m_chain.Tip(), *new_lock_points)) {
                ...
                m_mempool->mapTx->impl.modify(it.impl, [&new_lock_points](CTxMemPoolEntry& e) { e.UpdateLockPoints(*new_lock_points); });
            } else {
                return true;
            }
        }
