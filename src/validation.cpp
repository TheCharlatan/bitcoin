...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    for (const auto& txin : tx.vin) {
        const COutPoint& prevout = txin.prevout;
        const Coin& coin = m_view.AccessCoin(prevout);
        assert(!coin.IsSpent());
        spent_outputs.emplace_back(coin);
    }

    // The mempool holds txs for the next block, so pass height+1 to CheckTxInputs
    if (!Consensus::CheckTxInputs(tx, state, spent_outputs, m_active_chainstate.m_chain.Height() + 1, ws.m_base_fees)) {
    ...
    int64_t nSigOpsCost = GetTransactionSigOpCost(tx, spent_outputs, STANDARD_SCRIPT_VERIFY_FLAGS);
...
bool MemPoolAccept::PolicyScriptChecks(const ATMPArgs& args, Workspace& ws)
{
    ...
    if (!CheckInputScripts(tx, state, std::move(spent_outputs), scriptVerifyFlags, true, false, ws.m_precomputed_txdata, GetValidationCache())) {
        ...
        if (!tx.HasWitness() && CheckInputScripts(tx, state_dummy, std::move(spent_outputs), scriptVerifyFlags & ~(SCRIPT_VERIFY_WITNESS | SCRIPT_VERIFY_CLEANSTACK), true, false, ws.m_precomputed_txdata, GetValidationCache()) &&
                !CheckInputScripts(tx, state_dummy, std::move(spent_outputs), scriptVerifyFlags & ~SCRIPT_VERIFY_CLEANSTACK, true, false, ws.m_precomputed_txdata, GetValidationCache())) {
...
bool Chainstate::ConnectBlock(const CBlock& block, BlockValidationState& state, CBlockIndex* pindex,
                               CBlockUndo& blockundo, bool fJustCheck)
{
    AssertLockHeld(cs_main);
    assert(pindex);
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase())
        {
            std::vector<Coin>& spent_coins = blockundo.vtxundo[i-1].vprevout;
            CAmount txfee = 0;
        ...
        if (!tx.IsCoinBase()) {
            nSigOpsCost += GetTransactionSigOpCost(tx, blockundo.vtxundo[i-1].vprevout, flags);
        } else {
            std::vector<Coin> coin;
            nSigOpsCost += GetTransactionSigOpCost(tx, coin, flags);
        }
...
bool Chainstate::ConnectTip(BlockValidationState& state, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions& disconnectpool)
{
             ...
             Ticks<MillisecondsDouble>(time_2 - time_1));
    {
        ...
        // Special case for the genesis block, skipping connection of its transactions
        // (its coinbase is unspendable)
        if (blockConnecting.GetHash() != params.GetConsensus().hashGenesisBlock) {
            uint256 hashPrevBlock = pindexNew->pprev == nullptr ? uint256() : pindexNew->pprev->GetBlockHash();
            ...
            bool fEnforceBIP30 = !IsBIP30Repeat(*pindexNew);
            ...
            assert(pindexNew->pprev);
            CBlockIndex* pindexBIP34height = pindexNew->pprev->GetAncestor(params.GetConsensus().BIP34Height);
        ...
        for (const CTransactionRef& tx : blockConnecting.vtx) {
            blockundo.vtxundo.reserve(blockConnecting.vtx.size() - 1);
            if (!tx->IsCoinBase()) {
                blockundo.vtxundo.emplace_back();
            }
            CTxUndo dummy;
            UpdateCoins(*tx, view, tx->IsCoinBase() ? dummy : blockundo.vtxundo.back(), pindexNew->nHeight);

        }

        bool rv = ConnectBlock(blockConnecting, state, pindexNew, blockundo);
        if (m_chainman.m_options.signals) {
                       ...
                       bool fCheckMerkleRoot)
{
    ...
    for (const CTransactionRef& tx : block.vtx) {
        ...
        CTxUndo dummy;
        UpdateCoins(*tx, viewNew, tx->IsCoinBase() ? dummy : blockundo.vtxundo.back(), indexDummy.nHeight);
    }

    if (!chainstate.ConnectBlock(block, state, &indexDummy, blockundo, true)) {
        return false;
    }
    ...
    if (nCheckLevel >= 4 && !skipped_l3_checks) {
        while (pindex != chainstate.m_chain.Tip()) {
            ...
            for (const CTransactionRef& tx : block.vtx) {
                ...
                CTxUndo dummy;
                UpdateCoins(*tx, coins, tx->IsCoinBase() ? dummy : blockundo.vtxundo.back(), pindex->nHeight);
            }

            if (!chainstate.ConnectBlock(block, state, pindex, blockundo)) {
