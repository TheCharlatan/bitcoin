    ...
    EXCLUSIVE_LOCKS_REQUIRED(pool.cs, ::cs_main)
{
    ...
    pool.TrimToSize(pool.m_max_size, &vNoSpendsRemaining);
...
public:
    explicit MemPoolAccept(CTxMemPool& mempool, CChainState& active_chainstate) : m_pool(mempool), m_view(&m_dummy), m_viewmempool(&active_chainstate.CoinsTip(), m_pool), m_active_chainstate(active_chainstate),
                                                                                  ...
                                                                                  m_limit_ancestor_size(m_pool.m_limits.ancestor_size * 1000),
                                                                                  m_limit_descendants(m_pool.m_limits.descendant_count),
                                                                                  m_limit_descendant_size(m_pool.m_limits.descendant_size * 1000)
 {
    }
...
private:
    ...
    bool CheckFeeRate(size_t package_size, CAmount package_fee, TxValidationState& state) EXCLUSIVE_LOCKS_REQUIRED(::cs_main, m_pool.cs)
    {
        ...
        CAmount mempoolRejectFee = m_pool.GetMinFee(m_pool.m_max_size).GetFee(package_size);
...
CChainState::CChainState(
    ...
    : m_mempool(mempool),
      m_blockman(blockman),
      m_params(chainman.m_chainparams),
      m_chainman(chainman),
...
void InitScriptExecutionCache(std::optional<int64_t> max_size) {
    ...
    size_t nMaxCacheSize = std::min(std::max((int64_t)0, max_size.value_or(DEFAULT_MAX_SIG_CACHE_SIZE) / 2), MAX_MAX_SIG_CACHE_SIZE) * ((size_t) 1 << 20);
    ...
    int nManualPruneHeight)
{
    ...
    try {
    {
        ...
        if (fDoFullFlush || fPeriodicWrite) {
            ...
            if (!CheckDiskSpace(m_blockman.m_blocks_dir)) {
    ...
    const SnapshotMetadata& metadata)
{
    ...
    auto maybe_au_data = ExpectedAssumeutxo(base_height, m_chainparams);
    ...
    for (int i = AFTER_GENESIS_START; i <= snapshot_chainstate.m_chain.Height(); ++i) {
        ...
        if (DeploymentActiveAt(*index, m_chainparams.GetConsensus(), Consensus::DEPLOYMENT_SEGWIT)) {
