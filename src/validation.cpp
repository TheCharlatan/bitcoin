...
{
    AssertLockHeld(::cs_main);
    const int64_t nMempoolUsage = m_mempool ? m_chainman.GetNotifications().measureExternalDynamicMemoryUsage() : 0;
...
bool Chainstate::ActivateBestChainStep(BlockValidationState& state, CBlockIndex* pindexMostWork, const std::shared_ptr<const CBlock>& pblock, bool& fInvalidFound, ConnectTrace& connectTrace)
{
    ...
    while (m_chain.Tip() && m_chain.Tip() != pindexFork) {
        if (!DisconnectTip(state, &disconnectpool)) {
            ...
            if (m_mempool) m_mempool->MaybeUpdateMempoolForReorg(*this, disconnectpool, false);
    ...
    if (fBlocksDisconnected) {
        ...
        if (m_mempool) m_mempool->MaybeUpdateMempoolForReorg(*this, disconnectpool, true);
    }
...
bool Chainstate::InvalidateBlock(BlockValidationState& state, CBlockIndex* pindex)
{
    ...
    while (true) {
        ...
        if (m_mempool) m_mempool->MaybeUpdateMempoolForReorg(*this, disconnectpool, /* fAddToMempool = */ (++disconnected <= 10) && ret);
        if (!ret) return false;
