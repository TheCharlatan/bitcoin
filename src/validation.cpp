...
bool FatalError(BlockValidationState& state, const std::string& strMessage, Notifications& notifications, const bilingual_str& userMessage)
{
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (!CheckBlock(block, state, params.GetConsensus(), !fJustCheck, !fJustCheck)) {
        if (state.GetResult() == BlockValidationResult::BLOCK_MUTATED) {
            ...
            // problems.
            return FatalError(state, "Corrupt block found indicating potential hardware failure; shutting down", m_chainman.GetNotifications());
        }
    ...
    int nManualPruneHeight)
{
    ...
    try {
    {
        ...
        if (fDoFullFlush || fPeriodicWrite) {
            ...
            if (!CheckDiskSpace(gArgs.GetBlocksDirPath())) {
                return FatalError(state, "Disk space is too low!", m_chainman.GetNotifications(), _("Disk space is too low!"));
            }
            {
            ...
            // Then update all block file information (which may refer to block and undo files).
            {
                ...
                if (!m_blockman.WriteBlockIndexDB()) {
                    return FatalError(state, "Failed to write to block index database", m_chainman.GetNotifications());
                }
            }
        ...
        if (fDoFullFlush && !CoinsTip().GetBestBlock().IsNull()) {
            ...
            if (!CoinsTip().Flush())
                return FatalError(state, "Failed to write to coin database", m_chainman.GetNotifications());
            m_last_flush = nNow;
    ...
    if (full_flush_completed) {
        ...
        return FatalError(state, std::string("System error while flushing: ") + e.what(), m_chainman.GetNotifications());
    }
    return true;
}
        ...
        for (int bit = 0; bit < VERSIONBITS_NUM_BITS; bit++) {
            ...
            if (state == ThresholdState::ACTIVE || state == ThresholdState::LOCKED_IN) {
                ...
                if (state == ThresholdState::ACTIVE) {
                    m_chainman.GetNotifications().notifyWarning(warning);
                } else {
...
bool Chainstate::ConnectTip(BlockValidationState& state, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions& disconnectpool)
{
    ...
    if (!pblock) {
        ...
        if (!m_blockman.ReadBlockFromDisk(*pblockNew, *pindexNew)) {
            return FatalError(state, "Failed to read block", m_chainman.GetNotifications());
        }
...
bool Chainstate::ActivateBestChainStep(BlockValidationState& state, CBlockIndex* pindexMostWork, const std::shared_ptr<const CBlock>& pblock, bool& fInvalidFound, ConnectTrace& connectTrace)
{
    ...
    while (m_chain.Tip() && m_chain.Tip() != pindexFork) {
        if (!DisconnectTip(state, &disconnectpool)) {
            ...
            FatalError(state, "Failed to disconnect block; see debug.log for details", m_chainman.GetNotifications());
            return false;
        }
...
static bool NotifyHeaderTip(Chainstate& chainstate) LOCKS_EXCLUDED(cs_main) {
    ...
    if (fNotify) {
        chainstate.m_chainman.GetNotifications().notifyHeaderTip(GetSynchronizationState(fInitialBlockDownload), pindexHeader->nHeight, pindexHeader->nTime, false);
    }
    return fNotify;
}
    ...
    do {
...

        {
            ...
            if (pindexFork != pindexNewTip) {
                ...
                m_chainman.GetNotifications().notifyBlockTip(GetSynchronizationState(fInitialDownload), pindexNewTip);
            }
        }
...
bool Chainstate::InvalidateBlock(BlockValidationState& state, CBlockIndex* pindex)
{
    ...
    if (pindex_was_in_chain) {
        m_chainman.GetNotifications().notifyBlockTip(GetSynchronizationState(IsInitialBlockDownload()), to_mark_failed->pprev);
    }
    return true;
}
...
Notifications& ChainstateManager::GetNotifications() const
{
...
void ChainstateManager::ReportHeadersPresync(const arith_uint256& work, int64_t height, int64_t timestamp)
{
    ...
    GetNotifications().notifyHeaderTip(GetSynchronizationState(initial_download), height, timestamp, /*presync=*/true);
    if (initial_download) {
...
bool Chainstate::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, BlockValidationState& state, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock, bool min_pow_checked)
{
    ...
    try {
        ...
        return FatalError(state, std::string("System error: ") + e.what(), m_chainman.GetNotifications());
    }
...
{
    m_notifications.notifyProgress(_("Verifying blocks…").translated, 0, false);
}

CVerifyDB::~CVerifyDB()
{
    m_notifications.notifyProgress("", 100, false);
}
    ...
    int nCheckLevel, int nCheckDepth)
{
    ...
    for (pindex = chainstate.m_chain.Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        m_notifications.notifyProgress(_("Verifying blocks…").translated, percentageDone, false);
...
bool Chainstate::ReplayBlocks()
{
    ...
    m_chainman.GetNotifications().notifyProgress(_("Replaying blocks…").translated, 0, false);
    ...
    for (int nHeight = nForkHeight + 1; nHeight <= pindexNew->nHeight; ++nHeight) {
        ...
        m_chainman.GetNotifications().notifyProgress(_("Replaying blocks…").translated, (int)((nHeight - nForkHeight) * 100.0 / (pindexNew->nHeight - nForkHeight)), false);
        if (!RollforwardBlock(&pindex, cache)) return false;
    }

    cache.SetBestBlock(pindexNew->GetBlockHash());
    cache.Flush();
    m_chainman.GetNotifications().notifyProgress("", 100, false);
    return true;
}
        ...
        bool in_memory)
{
    ...
    if (!snapshot_ok) {
        ...
        if (auto snapshot_datadir = node::FindSnapshotChainstateDir()) {
            ...
            if (!removed) {
                this->GetNotifications().fatalError(strprintf("Failed to remove snapshot chainstate dir (%s). "
...
bool ChainstateManager::ValidatedSnapshotCleanup()
{
    ...
    auto rename_failed_abort = [this](
        ...
        this->GetNotifications().fatalError(strprintf(
            "Rename of '%s' -> '%s' failed. "
