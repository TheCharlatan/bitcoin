...
util::Result<bool, FatalCondition> Chainstate::ConnectTip(BlockValidationState& state, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions& disconnectpool)
{
    ...
    if (this != &m_chainman.ActiveChainstate()) {
        ...
        if (auto res = m_chainman.MaybeCompleteSnapshotValidation(); !res) {
            // TODO: Return an error or something so this continues to bubble up?
            FatalError(m_chainman.GetNotifications(), state, util::ErrorString(res).original);
        }
    }
...
util::Result<bool, FatalCondition> ChainstateManager::ProcessNewBlock(const std::shared_ptr<const CBlock>& block, bool force_processing, bool min_pow_checked, bool* new_block)
{
...

    {
        ...
        if (ret) {
            // Store to disk
            auto res{AcceptBlock(block, state, &pindex, force_processing, nullptr, new_block, min_pow_checked)};
            if (!res) {
                return res;
            }
            ret = res.value();
...
util::Result<void, FatalCondition> ChainstateManager::ValidatedSnapshotCleanup()
{
    ...
    if (!ibd_chainstate_path_maybe || !snapshot_chainstate_path_maybe) {
        ...
        return {util::Error{Untranslated("Cannot cleanup chainstate snapshot with in-memory chainstates.")}, FatalCondition::NoChainstatePaths};
    }
