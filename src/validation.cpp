...
bool MemPoolAccept::ConsensusScriptChecks(const ATMPArgs& args, Workspace& ws)
{
    ...
    unsigned int currentBlockScriptVerifyFlags{GetBlockScriptFlags(*m_active_chainstate.m_chain.Tip(), m_active_chainstate.m_chainparams.GetConsensus(), m_active_chainstate.m_versionbitscache)};
    ...
    const CoinsViewOptions& coins_view)
{
    if (m_from_snapshot_blockhash) {
        leveldb_dir+= node::SNAPSHOT_CHAINSTATE_SUFFIX;
    }
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (!m_chainman.AssumedValidBlock().IsNull()) {
        ...
        if (it != m_blockman.m_block_index.end()) {
            if (it->second.GetAncestor(pindex->nHeight) == pindex &&
                ...
                fScriptChecks = (GetBlockProofEquivalentTime(*m_blockman.m_best_header, *pindex, *m_blockman.m_best_header, params.GetConsensus()) <= 60 * 60 * 24 * 7 * 2);
            }
        }
    }
    ...
    unsigned int flags{GetBlockScriptFlags(*pindex, params.GetConsensus(), m_versionbitscache)};
    ...
    int nManualPruneHeight)
{
        ...
        if (fDoFullFlush || fPeriodicWrite) {
            ...
            // Then update all block file information (which may refer to block and undo files).
            {
                ...
                if (!m_blockman.WriteBlockIndexDB()) {
                    return FatalError(m_notifications, state, _("Failed to write to block index database."));
                }
            }
    ...
    if (full_flush_completed && m_signals) {
        // Update best block in wallet (so we can detect restored wallets).
        m_signals->ChainStateFlushed(this->GetRole(), m_chain.GetLocator());
    }
...
bool Chainstate::DisconnectTip(BlockValidationState& state, DisconnectedBlockTransactions* disconnectpool)
{
    ...
    if (m_signals) {
        m_signals->BlockDisconnected(pblock, pindexDelete);
    }
    return true;
}
...
bool Chainstate::ConnectTip(BlockValidationState& state, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions& disconnectpool)
{
             ...
             Ticks<MillisecondsDouble>(time_2 - time_1));
    {
        ...
        if (m_signals) {
            m_signals->BlockChecked(blockConnecting, state);
        }
        if (!rv) {
            if (state.IsInvalid())
    ...
    do {
...

        {
            ...
            do {
               ...
               const ChainstateRole chainstate_role{this->GetRole()};
                ...
                for (const PerBlockConnectTrace& trace : connectTrace.GetBlocksConnected()) {
                    ...
                    if (m_signals) {
                        m_signals->BlockConnected(chainstate_role, trace.pblock, trace.pindex);
                    }
                }
            ...
            if (this == &m_chainman.ActiveChainstate() && pindexFork != pindexNewTip) {
                ...
                if (kernel::IsInterrupted(m_notifications.blockTip(GetSynchronizationState(still_in_ibd, m_blockman.m_blockfiles_indexed), *pindexNewTip))) {
...
bool Chainstate::InvalidateBlock(BlockValidationState& state, CBlockIndex* pindex)
{
    ...
    if (pindex_was_in_chain) {
        ...
        // changes.
        (void)m_notifications.blockTip(GetSynchronizationState(m_chainman.IsInitialBlockDownload(), m_blockman.m_blockfiles_indexed), *to_mark_failed->pprev);
                       ...
                       bool fCheckMerkleRoot)
{
    ...
    if (!ContextualCheckBlockHeader(block, state, chainstate.m_blockman, chainparams.GetConsensus(), chainstate.m_versionbitscache, pindexPrev)) {
    ...
    if (!ContextualCheckBlock(block, state, chainparams.GetConsensus(), chainstate.m_versionbitscache, pindexPrev)) {
...
bool Chainstate::LoadChainTip()
{
    ...
    if (this->GetRole() != ChainstateRole::BACKGROUND) {
        ...
        (void)m_notifications.blockTip(GetSynchronizationState(/*init=*/true, m_blockman.m_blockfiles_indexed), *pindex);
    }
        ...
        bool in_memory)
{
...

    {
        ...
        snapshot_chainstate->InitCoinsDB(
            ...
            in_memory, false, m_options.datadir / "chainstate", m_options.coins_db, m_options.coins_view);
