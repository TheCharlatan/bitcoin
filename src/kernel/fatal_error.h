...
template <typename T>
[[nodiscard]] bool IsFatal(util::Result<T, FatalError>& result)
{
    return !result || !result.GetErrors().empty();
}

template <typename T>
[[nodiscard]] T UnwrapFatalError(util::Result<T, FatalError> result)
{
...
template <typename T>
[[nodiscard]] bool CheckFatalFailure(util::Result<T, kernel::FatalError> result, kernel::FatalError expected_failure)
{
