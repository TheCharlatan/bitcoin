...
struct CacheSizes {
    ...
    CacheSizes(int64_t total_cache)
    {
        ...
        coins_db = std::min({total_cache / 2, (total_cache / 4) + (1 << 23), MAX_COINS_DB_CACHE << 20}); // use 25%-50% of the remainder for disk cache
        total_cache -= coins_db;
        coins = total_cache; // the rest goes to in-memory cache
