...
const fs::path& BlockTreeStore::GetDataFilePath(ValueType value_type) const
{
    ...
    case ValueType::HEADER_DATA_END:
...
{
    {
        ...
        if (autofile.fclose() != 0) {
            throw BlockTreeStoreError(strprintf("Failed to close created header file %s", fs::PathToString(m_header_file_path)));
        }
    }
...
bool BlockTreeStore::WriteBatchSync(const std::vector<std::pair<int, const CBlockFileInfo*>>& fileInfo, int32_t last_file, const std::vector<CBlockIndex*>& blockinfo)
{
    ...
    size_t log_file_prealloc_size{fileInfo.size() * (BLOCK_FILE_INFO_WRAPPER_SIZE + FILE_POSITION_SIZE) + blockinfo.size() * block_index_entry_size};
    ...
    WriteValueType(log_file, ValueType::HEADER_DATA_END);
