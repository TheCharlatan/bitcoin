 ...
 *
 * By default the logger does not buffer previously generated log messages. This
 * can be enabled by compiling with `-DBUFFER_LOGS`.
 */
...
typedef enum {
    ...
    kernel_ERROR_UNKNOWN_CONTEXT_OPTION,
 ...
 */
typedef struct {
    int height; //!< Non-zero if the block is in the chain. Once retrieved not guaranteed to still be accurate.
 ...
 * used to validate taproot transactions if the taproot flag is set.
 *
...
int BITCOINKERNEL_WARN_UNUSED_RESULT kernel_verify_script(
    ...
    unsigned int flags,
    kernel_Error* error
) BITCOINKERNEL_ARG_NONNULL(1) BITCOINKERNEL_ARG_NONNULL(4) BITCOINKERNEL_ARG_NONNULL(6);
 ...
 *
 * This function will trigger an assertion if a logging connection already
 * exists.
 *
 ...
 *
 * @param[in] raw_block     Non-null, serialized block as a hexadecimal encoded string.
...
/**
 * @brief Return a block index info struct.
 *
 * @param[in] block_index Non-null.
