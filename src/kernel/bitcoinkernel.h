 ...
 * ------ Context ------
 *
 * The library provides a built-in constant kernel context. This context offers
 * only limited functionality. It detects and self-checks the correct sha256
 * implementation, initializes the random number generator and self-checks the
 * secp256k1 static context. It is used internally for otherwise "context-free"
 * operations.
 *
 ...
 *
 * The kernel context is used to initialize internal state, including
 * initializing entropy and initializing hardware-accelerated hashing. It is
...
typedef void (*kernel_NotifyWarning)(void* user_data, const char* warning);
 ...
 */
typedef enum {
    kernel_CHAIN_PARAMETERS_OPTION = 0,             //!< Set the chain parameters, value should be a valid pointer
...
typedef struct {
    ...
    kernel_NotifyWarning warning;        //!< A warning issued by the kernel library during validation.
...
/**
 * @brief Verify if the input at input_index of tx_to spends the script pubkey
 * under the constraints specified by flags. The additional amount parameter is
 * used for segwit transactions if the witness flag is set.
 *
 * @param[in] script_pubkey     Non-null, serialized script pubkey to be spent.
 * @param[in] script_pubkey_len Length of the script pubkey to be spent.
...
/**
 * @brief Verify if the input at input_index of tx_to spends the script pubkey
 * under the constraints specified by flags. The spent outputs parameter is used
 * to validate taproot transactions if the taproot flag is set.
 *
 * @param[in] script_pubkey     Non-null, serialized script pubkey to be spent.
 * @param[in] script_pubkey_len Length of the script pubkey to be spent.
 * @param[in] amount            Amount of the script pubkey's associated output.
...
/**
 * @brief Set the log level of the kernel logger.
 *
...
/**
 * @brief Start logging messages through the provided callback. Some log
 * messages produced before this function is called are buffered and on calling
 * this function are logged immediately. Buffered messages will have the default
 * logging format (not controlled by the logging options).
 *
 * @param[in] callback  Non-null, function through which messages will be logged.
 * @param[in] user_data Nullable, holds a user-defined opaque structure.
 ...
 */
void kernel_context_options_set(kernel_ContextOptions* context_options, const kernel_ContextOptionType n_option, const void* value, kernel_Error* error);
 ...
 */
kernel_ChainstateManagerOptions* kernel_chainstate_manager_options_create(const kernel_Context* context, const char* data_directory, kernel_Error* error);
 ...
 */
void kernel_chainstate_manager_destroy(kernel_ChainstateManager* chainstate_manager, const kernel_Context* context, kernel_Error* error);
 ...
 * @param[out] error               Nullable, will contain an error/success code for the operation.
 */
void kernel_validation_interface_register(kernel_Context* context, kernel_ValidationInterface* validation_interface, kernel_Error* error);
 ...
 * @param[out] error               Nullable, will contain an error/success code for the operation.
 */
void kernel_validation_interface_unregister(kernel_Context* context, kernel_ValidationInterface* validation_interface, kernel_Error* error);
 ...
 *
 * @param[in] context                 Non-null.
 * @param[in] chainstate_load_options Nullable, created by kernel_chainstate_load_options_create.
 ...
 *                       or 0 on error.
 */
uint64_t kernel_block_undo_size(kernel_BlockUndo* block_undo, kernel_Error* error);
 ...
 *                                   or 0 on error.
 */
uint64_t kernel_get_transaction_undo_size(kernel_BlockUndo* block_undo, uint64_t transaction_undo_index, kernel_Error* error);
 ...
 */
kernel_TransactionOutput* kernel_get_undo_output_by_index(kernel_BlockUndo* block_undo,
