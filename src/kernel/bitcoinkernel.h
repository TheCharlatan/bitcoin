 ...
 *
 * Similarly views only provide a view to the data they are constructed from.
 * The pointers used for their instantiation have to outlive any operations
 * done on the view.
 *
...
/**
 * Opaque data structure for holding a prepared view of arguments for verifying a script.
 */
typedef struct kernel_ScriptVerifyPreparedArgsView kernel_ScriptVerifyPreparedArgsView;
 ...
 */
typedef enum {
    kernel_SCRIPT_VERIFY_ARGS_OK = 0,
    kernel_SCRIPT_VERIFY_ARGS_ERROR_TX_INPUT_INDEX, //!< The provided input index is out of range of the actual number of inputs of the transaction.
    kernel_SCRIPT_VERIFY_ARGS_ERROR_INVALID_FLAGS, //!< The provided bitfield for the flags was invalid.
    kernel_SCRIPT_VERIFY_ARGS_ERROR_INVALID_FLAGS_COMBINATION, //!< The flags very combined in an invalid way.
    kernel_SCRIPT_VERIFY_ARGS_ERROR_SPENT_OUTPUTS_REQUIRED, //!< The taproot flag was set, so valid spent_outputs have to be provided.
    kernel_SCRIPT_VERIFY_ARGS_ERROR_SPENT_OUTPUTS_MISMATCH, //!< The number of spent outputs does not match the number of inputs of the tx.
...
/**
 * @brief Prepare the arguments required calling kernel_script_verify. If the
 ...
 * parameter is required to validate taproot transactions.
 *
 ...
 * @param[out] status           Nullable, will be set to an error code if there is an error.
 *                              Should be set to kernel_SCRIPT_VERIFY_ARGS_OK.
 * @return                      The prepared view or nullptr on error.
 */
kernel_ScriptVerifyPreparedArgsView* kernel_script_verify_prepared_args_create(
    const kernel_ScriptPubkey* script_pubkey,
    int64_t amount,
...
/**
 * Destroy the prepared args again.
 */
void kernel_script_verify_prepared_args_destroy(kernel_ScriptVerifyPreparedArgsView* args);

/**
 * @brief Verify if the preapred arguments from
 * kernel_script_verify_prepare_args's input at input_index of tx_to spends the
 * script pubkey under the constraints specified by flags.
 *
 * @param[in] prepared_args     Non-null, arguments as prepared by kernel_script_verify_prepare_args.
 * @return                      True if the script is valid.
 */
bool BITCOINKERNEL_WARN_UNUSED_RESULT kernel_script_verify(
    const kernel_ScriptVerifyPreparedArgsView* prepared_args
) BITCOINKERNEL_ARG_NONNULL(1);
