...
void kernel_context_options_set(kernel_ContextOptions* context_opts_, const kernel_ContextOptionType n_option, const void* value, kernel_Error* error)
{
    ...
    case kernel_ContextOptionType::kernel_TASK_RUNNER_CALLBACKS_OPTION: {
        ...
        context_opts->m_tr_cbs = std::make_unique<kernel_TaskRunnerCallbacks>(*tr_cbs);
        set_error_ok(error);
        return;
    }
    ...
    kernel_Error* error)
{
    ...
    default: {
        set_error(error, kernel_ErrorCode::kernel_ERROR_UNKNOWN_CHAINSTATE_LOAD_OPTION, "Unknown chainstate load option");
    }
    }
}
...
{
    auto block_index{reinterpret_cast<CBlockIndex*>(block_index_)};
