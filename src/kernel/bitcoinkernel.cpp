...
struct ScriptPubkey::ScriptPubkeyImpl {
    ...
    ScriptPubkeyImpl(CScript script_pubkey)
        : script_pubkey{std::move(script_pubkey)}
    {
    }
};
...
{
    return std::vector<unsigned char>(m_impl->script_pubkey.begin(), m_impl->script_pubkey.end());
}
...
struct TransactionOutput::TransactionOutputImpl {
    ...
    TransactionOutputImpl(CTxOut tx_out) : tx_output{tx_out} {}
};
...
{
    return ScriptPubkey(std::make_unique<ScriptPubkey::ScriptPubkeyImpl>(m_impl->tx_output.scriptPubKey));
}

int64_t TransactionOutput::GetOutputAmount() const noexcept
{
    return m_impl->tx_output.nValue;
}
...
bool BlockValidationState::IsValid() const noexcept
{
    return m_impl->m_block_validation_state.IsValid();
}

bool BlockValidationState::IsInvalid() const noexcept
{
    return m_impl->m_block_validation_state.IsInvalid();
}

bool BlockValidationState::IsError() const noexcept
{
    return m_impl->m_block_validation_state.IsError();
}

BlockValidationResult BlockValidationState::GetResult() const noexcept
{
    return m_impl->m_block_validation_state.GetResult();
}
