                          ...
                          btck_ScriptVerifyStatus* status)
{
    ...
    if (input_index >= tx.vin.size()) {
        if (status) *status = btck_SCRIPT_VERIFY_ERROR_TX_INPUT_INDEX;
        return false;
    }
...
{
    if (transaction_index >= block_spent_outputs->m_block_undo->vtxundo.size()) {
        LogInfo("transaction spent outputs index is out of bounds.");
        return nullptr;
    }
...
{
    if (coin_index >= transaction_spent_outputs->m_tx_undo->vprevout.size()) {
