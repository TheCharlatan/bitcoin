...
CBlockUndo* cast_cblock_undo(kernel_BlockUndo* block_undo, kernel_Error* err)
{
    if (!block_undo) {
        set_error_invalid_pointer(err, "Invalid kernel_BlockUndo pointer.");
        return nullptr;
    }
    return reinterpret_cast<CBlockUndo*>(block_undo);
}
                                                   ...
                                                   kernel_Error* error)
{
    ...
    auto res{chainman->m_blockman.UndoReadFromDisk(*block_undo, *block_index)};
    if (!res) {
...
uint64_t kernel_block_undo_size(kernel_BlockUndo* block_undo_, kernel_Error* error)
{
    auto block_undo{cast_cblock_undo(block_undo_, error)};
    if (!block_undo) {
        return 0;
    }
...
{
    delete reinterpret_cast<CBlockUndo*>(block_undo);
}

uint64_t kernel_get_transaction_undo_size(kernel_BlockUndo* block_undo_, uint64_t transaction_undo_index, kernel_Error* error)
{
    auto block_undo{cast_cblock_undo(block_undo_, error)};
    if (!block_undo) {
        set_error_invalid_pointer(error, "Invalid kernel_BlockUndo pointer.");
        return 0;
    }
