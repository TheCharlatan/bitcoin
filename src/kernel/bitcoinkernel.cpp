...
struct ScriptVerifyPreparedArgsView {
...
kernel_ScriptVerifyPreparedArgsView* kernel_script_verify_prepared_args_create(
    const kernel_ScriptPubkey* script_pubkey_,
    int64_t amount_,
    ...
    kernel_ScriptVerifyStatus* status)
{
    ...
    if (!verify_flags(flags)) {
        if (status) *status = kernel_SCRIPT_VERIFY_ARGS_ERROR_INVALID_FLAGS;
        return nullptr;
    }

    if (!is_valid_flag_combination(flags)) {
        if (status) *status = kernel_SCRIPT_VERIFY_ARGS_ERROR_INVALID_FLAGS_COMBINATION;
        return nullptr;
    }

    if (flags & kernel_SCRIPT_FLAGS_VERIFY_TAPROOT && spent_outputs_ == nullptr) {
        if (status) *status = kernel_SCRIPT_VERIFY_ARGS_ERROR_SPENT_OUTPUTS_REQUIRED;
        return nullptr;
    }
    ...
    if (spent_outputs_ != nullptr) {
        if (spent_outputs_len != tx.vin.size()) {
            if (status) *status = kernel_SCRIPT_VERIFY_ARGS_ERROR_SPENT_OUTPUTS_MISMATCH;
            return nullptr;
        }
    ...
    if (input_index >= tx.vin.size()) {
        if (status) *status = kernel_SCRIPT_VERIFY_ARGS_ERROR_TX_INPUT_INDEX;
        return nullptr;
    }

    auto args = new ScriptVerifyPreparedArgsView {
        tx, flags, input_index, PrecomputedTransactionData{tx}, script_pubkey, amount
    };

    if (spent_outputs_ != nullptr && flags & kernel_SCRIPT_FLAGS_VERIFY_TAPROOT) {
        args->txdata.Init(tx, std::move(spent_outputs));
    }

    if (status) *status = kernel_SCRIPT_VERIFY_ARGS_OK;

    return reinterpret_cast<kernel_ScriptVerifyPreparedArgsView*>(args);
}

void kernel_script_verify_prepared_args_destroy(kernel_ScriptVerifyPreparedArgsView* args)
{
    if (args) {
        delete reinterpret_cast<ScriptVerifyPreparedArgsView*>(args);
    }
}

bool kernel_script_verify(const kernel_ScriptVerifyPreparedArgsView* args_)
{
    auto args{reinterpret_cast<const ScriptVerifyPreparedArgsView*>(args_)};

    return VerifyScript(args->tx.vin[args->input_index].scriptSig,
                        args->script_pubkey,
                        &args->tx.vin[args->input_index].scriptWitness,
                        args->flags,
                        TransactionSignatureChecker(&args->tx, args->input_index, args->amount, args->txdata, MissingDataBehavior::FAIL),
                        nullptr);
}
