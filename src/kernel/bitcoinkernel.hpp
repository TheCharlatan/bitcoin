...
public:
    Logger(std::function<void(std::string_view)> callback, const kernel_LoggingOptions& logging_options) noexcept;
    ~Logger();
...
public:
	UnownedBlock(std::unique_ptr<UnownedBlockImpl> impl) noexcept;
...
public:
	BlockValidationState(std::unique_ptr<BlockValidationStateImpl> impl) noexcept;
    ...
    BlockValidationResult GetResult() const noexcept;
...
public:
    Context(const ContextOptions& opts) noexcept;
    Context() noexcept;
    ~Context() noexcept;
...
public:
    ...
    Block(std::unique_ptr<BlockImpl> impl) noexcept;
    ~Block() noexcept;
