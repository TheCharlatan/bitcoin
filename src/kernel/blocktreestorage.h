...
//! <magic> <version> <last block position> <checksum> [<BlockFileInfoWrapper> <checksum>]
...
private:
    ...
    AutoFile OpenBlockFilesFile(std::string mode) const EXCLUSIVE_LOCKS_REQUIRED(m_mutex);
...
public:
    ...
    void WriteBatchSync(const std::vector<std::pair<int, const CBlockFileInfo*>>& fileInfo, int32_t last_file, const std::vector<CBlockIndex*>& blockinfo)
