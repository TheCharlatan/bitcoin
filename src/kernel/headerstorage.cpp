...
static void WriteReindexing(AutoFile& file, bool fReindexing)
{
...
static void WritePruned(AutoFile& file, bool prune)
{
...
void BlockTreeStore::WriteReindexing(bool reindexing) const
{
    ...
    ::WriteReindexing(file, reindexing);
}
    ...
    bool check_pow)
{
    ...
    while (file.tell() < data_end_pos) {
        ...
        if (check_pow && !CheckProofOfWork(pindexNew->GetBlockHash(), pindexNew->nBits, consensusParams)) {
