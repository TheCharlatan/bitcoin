...
{
public:
    explicit BlockHeader(std::span<const std::byte> raw_block_header)
        : Handle{btck_block_header_create(raw_block_header.data(), raw_block_header.size())} {}
...
public:
    ...
    BlockHeader GetHeader() const
    {
        return BlockHeaderView{btck_block_tree_entry_get_block_header(get())};
    }
...
public:
    ...
    {
        return btck_chainstate_manager_process_block_header(get(), header.get(), state.get(), nullptr) == 0;
    }
