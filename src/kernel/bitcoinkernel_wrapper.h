...
template <typename T>
std::vector<std::byte> write_bytes(const T* object, bool (*to_bytes)(const T*, btck_WriteBytes, void*))
{
    ...
    if (!to_bytes(object, write, &user_data)) {
...
public:
    ...
    {
        auto index{btck_chain_get_next_block_index(m_chain.get(), block_index.m_block_tree_entry.get())};
        if (!index) return std::nullopt;
        return index;
    }
