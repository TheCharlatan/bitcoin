                  ...
                  kernel_ScriptVerifyStatus& status) noexcept
{
    ...
    kernel_ScriptVerifyPreparedArgsView* args = kernel_script_verify_prepared_args_create(
        ...
        &status);
    if (!args) return 0;
    auto res = kernel_script_verify(args);
    kernel_script_verify_prepared_args_destroy(args);
    return res;
}
