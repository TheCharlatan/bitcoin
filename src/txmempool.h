...
public:
    ...
    void addUnchecked(const CTxMemPoolEntry& entry, setEntryRefs& setAncestors, bool validFeeEstimate = true) EXCLUSIVE_LOCKS_REQUIRED(cs, cs_main);
    ...
    setEntryRefs GetEntrySet(const std::set<uint256>& hashes) const EXCLUSIVE_LOCKS_REQUIRED(cs);
    ...
    bool BuildClusterForTransaction(CTxMemPoolEntry& entry, const setEntryRefs& all_conflicts, const Limits& limits, Cluster& temp_cluster) EXCLUSIVE_LOCKS_REQUIRED(cs);

    util::Result<CFeeRate> CalculateMiningScoreOfReplacementTx(CTxMemPoolEntry& entry, CAmount modified_fee,
                                                 const setEntryRefs& all_conflicts, const Limits& limits) EXCLUSIVE_LOCKS_REQUIRED(cs);
...
private:
    ...
    void UpdateParentsOf(bool add, const CTxMemPoolEntry& hash) EXCLUSIVE_LOCKS_REQUIRED(cs);
    /** For each transaction being removed, update parents and any children. */
    void UpdateForRemoveFromMempool(const setEntryRefs& entriesToRemove) EXCLUSIVE_LOCKS_REQUIRED(cs);
