...
bool CoinStatsIndex::CustomAppend(const interfaces::BlockInfo& block)
{
    ...
    if (block.height > 0) {
        ...
        if (!UndoReadFromDisk(m_chainstate->BlocksDirPath(), block_undo, pindex)) {
            return false;
        }
...
bool CoinStatsIndex::CustomRewind(const interfaces::BlockKey& current_tip, const interfaces::BlockKey& new_tip)
{
...

    {
        ...
        do {
            ...
            if (!ReadBlockFromDisk(m_chainstate->BlocksDirPath(), m_chainstate->FastPrune(), block, iter_tip, consensus_params)) {
