...
arith_uint256 CalculateClaimedHeadersWork(const std::span<CBlockHeader> headers);
...
public:
     ...
     */
    bool ProcessNewBlockHeaders(const std::span<CBlockHeader> headers, bool min_pow_checked, BlockValidationState& state, const CBlockIndex** ppindex = nullptr) LOCKS_EXCLUDED(cs_main);
