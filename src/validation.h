...
[[nodiscard]] util::Result<void, FatalCondition> PruneBlockFilesManual(Chainstate& active_chainstate, int nManualPruneHeight);
 ...
 */
[[nodiscard]] util::Result<MempoolAcceptResult, FatalCondition> AcceptToMemoryPool(Chainstate& active_chainstate, const CTransactionRef& tx,
...
*/
[[nodiscard]] util::Result<PackageMempoolAcceptResult, FatalCondition> ProcessNewPackage(Chainstate& active_chainstate, CTxMemPool& pool,
...
[[nodiscard]] util::Result<bool, FatalCondition> TestBlockValidity(BlockValidationState& state,
...
public:
    ...
    ~CVerifyDB();
    [[nodiscard]] util::Result<VerifyDBResult, FatalCondition> VerifyDB(
        Chainstate& chainstate,
...
public:
    ...
    [[nodiscard]] util::Result<bool, FatalCondition> ResizeCoinsCaches(size_t coinstip_size, size_t coinsdb_size)
        EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
     ...
     */
    [[nodiscard]] util::Result<bool, FatalCondition> FlushStateToDisk(
        BlockValidationState& state,
        FlushStateMode mode,
        int nManualPruneHeight = 0);

    //! Unconditionally flush all changes to disk.
    [[nodiscard]] util::Result<void, FatalCondition> ForceFlushStateToDisk();

    //! Prune blockfiles from the disk if necessary and then flush chainstate changes
    //! if we pruned.
    [[nodiscard]] util::Result<void, FatalCondition> PruneAndFlush();
     ...
     * @returns true unless a system error occurred
     */
    [[nodiscard]] util::Result<bool, FatalCondition> ActivateBestChain(
        BlockValidationState& state,
        std::shared_ptr<const CBlock> pblock = nullptr)
        EXCLUSIVE_LOCKS_REQUIRED(!m_chainstate_mutex)
        LOCKS_EXCLUDED(::cs_main);

    [[nodiscard]] util::Result<bool, FatalCondition> AcceptBlock(const std::shared_ptr<const CBlock>& pblock, BlockValidationState& state, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock, bool min_pow_checked) EXCLUSIVE_LOCKS_REQUIRED(cs_main);

    // Block (dis)connection on a given view:
    DisconnectResult DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view)
        EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
    [[nodiscard]] util::Result<bool, FatalCondition> ConnectBlock(const CBlock& block, BlockValidationState& state, CBlockIndex* pindex,
                      CCoinsViewCache& view, bool fJustCheck = false) EXCLUSIVE_LOCKS_REQUIRED(cs_main);

    // Apply the effects of a block disconnection on the UTXO set.
    [[nodiscard]] util::Result<bool, FatalCondition> DisconnectTip(BlockValidationState& state, DisconnectedBlockTransactions* disconnectpool) EXCLUSIVE_LOCKS_REQUIRED(cs_main, m_mempool->cs);
     ...
     */
    [[nodiscard]] util::Result<bool, FatalCondition> PreciousBlock(BlockValidationState& state, CBlockIndex* pindex)
        EXCLUSIVE_LOCKS_REQUIRED(!m_chainstate_mutex)
        LOCKS_EXCLUDED(::cs_main);

    /** Mark a block as invalid. */
    [[nodiscard]] util::Result<bool, FatalCondition> InvalidateBlock(BlockValidationState& state, CBlockIndex* pindex)
        EXCLUSIVE_LOCKS_REQUIRED(!m_chainstate_mutex)
        LOCKS_EXCLUDED(::cs_main);
    ...
    [[nodiscard]] util::Result<bool, FatalCondition> LoadGenesisBlock();
    ...
    [[nodiscard]] util::Result<void, FatalCondition> LoadMempool(const fs::path& load_path, fsbridge::FopenFn mockable_fopen_function = fsbridge::fopen);
...
private:
    [[nodiscard]] util::Result<bool, FatalCondition> ActivateBestChainStep(BlockValidationState& state, CBlockIndex* pindexMostWork, const std::shared_ptr<const CBlock>& pblock, bool& fInvalidFound, ConnectTrace& connectTrace) EXCLUSIVE_LOCKS_REQUIRED(cs_main, m_mempool->cs);
    [[nodiscard]] util::Result<bool, FatalCondition> ConnectTip(BlockValidationState& state, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions& disconnectpool) EXCLUSIVE_LOCKS_REQUIRED(cs_main, m_mempool->cs);
     ...
     */
    [[nodiscard]] util::Result<void, FatalCondition> MaybeUpdateMempoolForReorg(
     ...
     */
    [[nodiscard]] util::Result<void, FatalCondition> InvalidateCoinsDBOnDisk() EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
...
public:
    ...
    [[nodiscard]] util::Result<bool, FatalCondition> ActivateSnapshot(
     ...
     */
    [[nodiscard]] util::Result<bool, FatalCondition> ProcessNewBlock(const std::shared_ptr<const CBlock>& block, bool force_processing, bool min_pow_checked, bool* new_block) LOCKS_EXCLUDED(cs_main);
     ...
     * @param[in]  test_accept     When true, run validation checks but don't submit to mempool.
     */
    [[nodiscard]] util::Result<MempoolAcceptResult, FatalCondition> ProcessTransaction(const CTransactionRef& tx, bool test_accept=false)
        EXCLUSIVE_LOCKS_REQUIRED(cs_main);
    ...
    [[nodiscard]] util::Result<void, FatalCondition> MaybeRebalanceCaches() EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
    ...
    [[nodiscard]] util::Result<void, FatalCondition> ValidatedSnapshotCleanup() EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
