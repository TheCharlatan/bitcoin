...
enum class SnapshotCompletionResult {
    ...
    // The blockhash of the current tip of the validated chainstate does not
    // match the one expected by the snapshot chainstate.
 ...
 * and contains database of UTXOs existing at a different point in history. (See
 * \ref Chainstate class for more information.)
 *
 ...
 * there will be two chainstates. The normal, fully validated chainstate will
 ...
 * chainstate will be used as the chain tip for the wallet and RPCs
 * even though it is only assumed-valid. When the initial
 * chainstate catches up to the snapshot height and confirms that the
 * assume-valid snapshot is actually valid, the assumed-valid chainstate will be
 * confirmed fully-valid and become the only chainstate again.
 */
class ChainstateManager
{
private:
...
public:
    ...
    //! Once a fully validated chainstate reaches an assumeutxo
    //! snapshot block, the snapshot can be validated. If the snapshot hash
    //! matches the validated UTXO set, then the chainstate based on the snapshot
    ...
    Chainstate& CurrentChainstate() const EXCLUSIVE_LOCKS_REQUIRED(GetMutex())
    {
        for (auto& cs : m_chainstates) {
            if (cs && cs->Validity() != ChainValidity::INVALID && !cs->m_target_blockhash) return *cs;
        }
        abort();
    }
    ...
    {
        for (auto& cs : m_chainstates) {
            if (cs && cs->Validity() != ChainValidity::INVALID && cs->m_target_blockhash && !cs->m_target_utxohash) return cs.get();
        }
        return nullptr;
    }
    ...
    Chainstate& ValidatedChainstate() const EXCLUSIVE_LOCKS_REQUIRED(GetMutex())
    {
        for (auto* cs : {&CurrentChainstate(), HistoricalChainstate()}) {
            if (cs && cs->Validity() == ChainValidity::VALIDATED) return *cs;
        }
        abort();
    }
    ...
    bool ValidatedSnapshotCleanup(Chainstate& validated_chainstate, Chainstate& from_snapshot_chainstate) EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
    ...
    //! List of chainstates. Note: in general, it is not safe delete Chainstate
    //! objects once they are added to this list because there is no mutex that
    //! can be locked to prevent Chainstate pointers from being used while they
    //! are deleted. (cs_main doesn't work because it is too narrow and is
    //! released in the middle of Chainstate::ActivateBestChain to let
