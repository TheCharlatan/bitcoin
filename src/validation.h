...
arith_uint256 CalculateClaimedHeadersWork(const std::span<const CBlockHeader> headers);
...
public:
     ...
     */
    bool ProcessNewBlockHeaders(const std::span<const CBlockHeader> headers, bool min_pow_checked, BlockValidationState& state, const CBlockIndex** ppindex = nullptr) LOCKS_EXCLUDED(cs_main);
