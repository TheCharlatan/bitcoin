...
std::unique_ptr<const CChainParams> CreateSignetChainParams(const ArgsManager& args)
{
    ...
    if (args.IsArgSet("-signetseednode")) {
        auto specified_seeds = args.GetArgs("-signetseednode");
        seeds.insert(seeds.end(), specified_seeds.begin(), specified_seeds.end());
    }
...
std::unique_ptr<const CChainParams> CreateRegTestChainParams(const ArgsManager& args)
{
    ...
    auto rv = CreateRegTestChainParams(RegTestOptions{version_bits_parameters, activation_heights, prune_after_height});
