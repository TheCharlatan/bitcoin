...
void PeerManagerImpl::ProcessGetBlockData(CNode& pfrom, Peer& peer, const CInv& inv)
{
    ...
    if (a_recent_block && a_recent_block->GetHash() == pindex->GetBlockHash()) {
        ...
        if (!ReadRawBlockFromDisk(m_chainman.BlocksDirPath(), block_data, pindex->GetBlockPos(), m_chainparams.MessageStart())) {
        ...
        if (!ReadBlockFromDisk(m_chainman.BlocksDirPath(), *pblockRead, pindex, m_chainparams.GetConsensus())) {
            assert(!"cannot load block from disk");
        }
        pblock = pblockRead;
    }
    ...
    if (msg_type == NetMsgType::GETBLOCKTXN) {
...

        {
            ...
            if (pindex->nHeight >= m_chainman.ActiveChain().Height() - MAX_BLOCKTXN_DEPTH) {
                CBlock block;
                bool ret = ReadBlockFromDisk(m_chainman.BlocksDirPath(), block, pindex, m_chainparams.GetConsensus());
                assert(ret);
            ...
            if (!fRevertToInv && !vHeaders.empty()) {
                if (vHeaders.size() == 1 && state.m_requested_hb_cmpctblocks) {
                    ...
                    if (cached_cmpctblock_msg.has_value()) {
                        ...
                        CBlock block;
                        bool ret = ReadBlockFromDisk(m_chainman.BlocksDirPath(), block, pBestIndex, consensusParams);
                        assert(ret);
