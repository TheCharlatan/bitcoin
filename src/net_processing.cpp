                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::CMPCTBLOCK)
    {
        ...
        if (!prev_block) {
            ...
            return;
        } else if (prev_block->nChainWork + CalculateClaimedHeadersWork(std::span<CBlockHeader>{&cmpctblock.header, 1}) < GetAntiDoSWorkThreshold()) {
        ...
        const CBlockIndex *pindex = nullptr;
        BlockValidationState state;
        if (!m_chainman.ProcessNewBlockHeaders(std::span<CBlockHeader>{&cmpctblock.header, 1}, /*min_pow_checked=*/true, state, &pindex)) {
            if (state.IsInvalid()) {
    ...
    if (msg_type == NetMsgType::BLOCK)
    {
        ...
        bool min_pow_checked = false;
        {
            ...
            if (prev_block && prev_block->nChainWork + CalculateClaimedHeadersWork(std::span<CBlockHeader>{&header, 1}) >= GetAntiDoSWorkThreshold()) {
