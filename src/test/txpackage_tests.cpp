...
BOOST_FIXTURE_TEST_CASE(package_validation_tests, TestChain100Setup)
{
    ...
    const auto result_parent_child = UnwrapFatalError(ProcessNewPackage(m_node.chainman->ActiveChainstate(), *m_node.mempool, package_parent_child, /*test_accept=*/true));
    ...
    auto result_single_large = UnwrapFatalError(ProcessNewPackage(m_node.chainman->ActiveChainstate(), *m_node.mempool, package_single_giant, /*test_accept=*/true));
...
BOOST_FIXTURE_TEST_CASE(package_witness_swap_tests, TestChain100Setup)
{
    ...
    {
        const auto mixed_result = UnwrapFatalError(ProcessNewPackage(m_node.chainman->ActiveChainstate(), *m_node.mempool, package_mixed, false));
