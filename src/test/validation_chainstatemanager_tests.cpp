...
BOOST_FIXTURE_TEST_CASE(chainstatemanager, TestChain100Setup)
{
    ...
    BOOST_CHECK(WITH_LOCK(::cs_main, return manager.CurrentChainstate().Validity() == ChainValidity::ASSUMED_VALID));
...
BOOST_FIXTURE_TEST_CASE(chainstatemanager_snapshot_init, SnapshotTestSetup)
{
...

    {
        ...
        BOOST_CHECK(chainman_restarted.CurrentChainstate().Validity() == ChainValidity::ASSUMED_VALID);
...
BOOST_FIXTURE_TEST_CASE(chainstatemanager_snapshot_completion, SnapshotTestSetup)
{
    ...
    res = WITH_LOCK(::cs_main, return chainman.MaybeCompleteSnapshotValidation(validated_cs, active_cs));
    BOOST_CHECK_EQUAL(res, SnapshotCompletionResult::SUCCESS);

    BOOST_CHECK(WITH_LOCK(::cs_main, return chainman.CurrentChainstate().Validity() == ChainValidity::VALIDATED));
    BOOST_CHECK(WITH_LOCK(::cs_main, return chainman.CurrentChainstate().m_from_snapshot_blockhash));
    BOOST_CHECK_EQUAL(WITH_LOCK(::cs_main, return chainman.HistoricalChainstate()), nullptr);
...
BOOST_FIXTURE_TEST_CASE(chainstatemanager_snapshot_completion_hash_mismatch, SnapshotTestSetup)
{
    ...
    Chainstate& from_snapshot_chainstate = *std::get<1>(chainstates);
    ChainstateManager& chainman = *Assert(m_node.chainman);
    SnapshotCompletionResult res;
    m_node.notifications->m_shutdown_on_fatal_error = false;
...

    {
        ...
        res = WITH_LOCK(::cs_main, return chainman.MaybeCompleteSnapshotValidation(validation_chainstate, from_snapshot_chainstate));
...

    {
        ...
        BOOST_CHECK(chainman.m_chainstates[0]->Validity() == ChainValidity::VALIDATED);
        BOOST_CHECK(!chainman.m_chainstates[0]->SnapshotBase());
        BOOST_CHECK(chainman.m_chainstates[1]->Validity() == ChainValidity::INVALID);
