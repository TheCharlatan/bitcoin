...
BOOST_AUTO_TEST_CASE(validation_chainstate_resize_caches)
{
    ...
    BOOST_REQUIRE(c1.LoadGenesisBlock().value()); // Need at least one block loaded to be able to flush caches
...
BOOST_FIXTURE_TEST_CASE(chainstate_update_tip, TestChain100Setup)
{
    ...
    // once it is changed to support multiple chainstates.
    {
        ...
        BOOST_CHECK(checked);
        bool accepted = Assert(chainman.AcceptBlock(
            pblockone, state, &pindex, true, nullptr, &newblock, true)).value();
        BOOST_CHECK(accepted);
    }

    // UpdateTip is called here
    bool block_added = Assert(background_cs.ActivateBestChain(state, pblockone)).value();
