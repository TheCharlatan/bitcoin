...
BOOST_FIXTURE_TEST_CASE(miniminer_1p1c, TestChain100Setup)
{
    ...
    for (const auto& tx : all_transactions) {
        const CTxMemPoolEntry& entry = pool.GetEntry(tx->GetHash()).value();
 ...
 *  -> tx7_ASFR = (high+low+high) / (tx4_vsize + tx5_vsize + tx7_vsize) */
    ...
    const CTxMemPoolEntry& tx3_entry = pool.GetEntry(tx3->GetHash()).value();
    ...
    const CTxMemPoolEntry& tx6_entry = pool.GetEntry(tx6->GetHash()).value();
    ...
    const CTxMemPoolEntry& tx7_entry = pool.GetEntry(tx7->GetHash()).value();
