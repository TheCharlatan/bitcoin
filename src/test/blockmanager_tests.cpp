...
BOOST_AUTO_TEST_CASE(blockmanager_find_block_pos)
{
    ...
    BOOST_CHECK_EQUAL(Assert(blockman.SaveBlockToDisk(params->GenesisBlock(), 0, chain, nullptr)).value().nPos, BLOCK_SERIALIZATION_HEADER_SIZE);
    // simulate what happens during reindex
    // simulate a well-formed genesis block being found at offset 8 in the blk00000.dat file
    // the block is found at offset 8 because there is an 8 byte serialization header
    // consisting of 4 magic bytes + 4 length bytes before each block in a well-formed blk file.
    FlatFilePos pos{0, BLOCK_SERIALIZATION_HEADER_SIZE};
    BOOST_CHECK_EQUAL(Assert(blockman.SaveBlockToDisk(params->GenesisBlock(), 0, chain, &pos)).value().nPos, BLOCK_SERIALIZATION_HEADER_SIZE);
    ...
    FlatFilePos actual{Assert(blockman.SaveBlockToDisk(params->GenesisBlock(), 1, chain, nullptr)).value()};
