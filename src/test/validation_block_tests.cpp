...
BOOST_AUTO_TEST_CASE(processnewblock_signals_ordering)
{
    ...
    BOOST_CHECK(Assert(Assert(m_node.chainman)->ProcessNewBlock(std::make_shared<CBlock>(Params().GenesisBlock()), true, true, &ignored)).value());
    ...
    for (int i = 0; i < 10; i++) {
        threads.emplace_back([&]() {
            ...
            for (int i = 0; i < 1000; i++) {
                ...
                Assert(Assert(m_node.chainman)->ProcessNewBlock(block, true, true, &ignored));
            }

            // to make sure that eventually we process the full chain - do it here
            for (const auto& block : blocks) {
                if (block->vtx.size() == 1) {
                    auto processed = Assert(m_node.chainman)->ProcessNewBlock(block, true, true, &ignored);
                    assert(Assert(processed).value());
                }
            }
        });
    }
...
BOOST_AUTO_TEST_CASE(mempool_locks_reorg)
{
    ...
    auto ProcessBlock = [&](std::shared_ptr<const CBlock> block) -> bool {
        return Assert(Assert(m_node.chainman)->ProcessNewBlock(block, /*force_processing=*/true, /*min_pow_checked=*/true, /*new_block=*/&ignored)).value();
    };
    ...
    for (int test_runs = 3; test_runs > 0; --test_runs) {
        ...
        // Add the txs to the tx pool
        {
            ...
            for (const auto& tx : txs) {
                const MempoolAcceptResult result = Assert(m_node.chainman->ProcessTransaction(tx)).value();
