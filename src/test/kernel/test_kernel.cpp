    ...
    bool taproot)
{
    ...
    auto status = kernel_ScriptVerifyStatus::kernel_SCRIPT_VERIFY_ARGS_OK;
    ...
    if (taproot) {
        verify_script(
            ...
            status);
        assert(status == kernel_SCRIPT_VERIFY_ARGS_OK);
    } else {
        assert(!verify_script(
            ...
            kernel_SCRIPT_FLAGS_VERIFY_ALL,
            status));
        assert(status == kernel_SCRIPT_VERIFY_ARGS_ERROR_SPENT_OUTPUTS_REQUIRED);
        status = kernel_SCRIPT_VERIFY_ARGS_OK;
    }
    ...
    assert(!verify_script(
        ...
        status));
    assert(status == kernel_SCRIPT_VERIFY_ARGS_ERROR_INVALID_FLAGS);
    ...
    assert(!verify_script(
        ...
        VERIFY_ALL_PRE_TAPROOT,
        status));
    assert(status == kernel_SCRIPT_VERIFY_ARGS_ERROR_TX_INPUT_INDEX);
    status = kernel_SCRIPT_VERIFY_ARGS_OK;
}
