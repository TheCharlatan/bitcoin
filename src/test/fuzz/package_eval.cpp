...
CTxMemPool MakeMempool(FuzzedDataProvider& fuzzed_data_provider, const NodeContext& node)
{
    ...
    return CTxMemPool{*node.validation_signals, mempool_opts};
}
...
FUZZ_TARGET(tx_package_eval, .init = initialize_tx_pool)
{
    ...
    LIMITED_WHILE(fuzzed_data_provider.ConsumeBool(), 300)
    {
        ...
        g_setup->m_node.validation_signals->SyncWithValidationInterfaceQueue();
        g_setup->m_node.validation_signals->UnregisterSharedValidationInterface(txr);
    ...
    g_setup->m_node.validation_signals->UnregisterSharedValidationInterface(outpoints_updater);
