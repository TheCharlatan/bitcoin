...
static void CheckSort(CTxMemPool& pool, std::vector<std::string>& sortedOrder) EXCLUSIVE_LOCKS_REQUIRED(pool.cs)
{
    ...
    typename MempoolMultiIndex::indexed_transaction_set::index<name>::type::iterator it = pool.mapTx->impl.get<name>().begin();
    int count = 0;
...
BOOST_AUTO_TEST_CASE(MempoolIndexingTest)
{
    ...
    setAncestors.insert(pool.mapTx->impl.find(tx6.GetHash()));
    ...
    setAncestors.insert(pool.mapTx->impl.find(tx7.GetHash()));
    ...
    setAncestors.insert(pool.mapTx->impl.find(tx8.GetHash()));
    setAncestors.insert(pool.mapTx->impl.find(tx9.GetHash()));
