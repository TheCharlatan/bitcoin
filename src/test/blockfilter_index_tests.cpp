                               ...
                               uint256& last_header)
{
    ...
    if (!ComputeFilter(blockman, filter_index.GetFilterType(), block_index, expected_filter)) {
...
BOOST_FIXTURE_TEST_CASE(blockfilter_index_initial_sync, BuildChainTestingSetup)
{
    ...
    // Check that filter index has all blocks that were in the chain before it started.
    {
        ...
        for (block_index = m_node.chainman->ActiveChain().Genesis();
             block_index != nullptr;
             block_index = m_node.chainman->ActiveChain().Next(block_index)) {
            CheckFilterLookups(m_node.chainman->m_blockman, filter_index, block_index, last_header);
        }
    }
    ...
    for (size_t i = 0; i < 2; i++) {
        ...
        BOOST_CHECK(filter_index.BlockUntilSyncedToCurrentChain());
        CheckFilterLookups(m_node.chainman->m_blockman, filter_index, block_index, chainA_last_header);
    }
    ...
    for (size_t i = 0; i < 3; i++) {
        ...
        BOOST_CHECK(filter_index.BlockUntilSyncedToCurrentChain());
        CheckFilterLookups(m_node.chainman->m_blockman, filter_index, block_index, chainB_last_header);
    }
