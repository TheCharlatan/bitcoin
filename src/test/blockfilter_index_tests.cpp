...
static bool CheckFilterLookups(const fs::path& blocks_dir, const bool fast_prune, BlockFilterIndex& filter_index, const CBlockIndex* block_index,
                               uint256& last_header)
{
    BlockFilter expected_filter;
    if (!ComputeFilter(blocks_dir, fast_prune, filter_index.GetFilterType(), block_index, expected_filter)) {
...
BOOST_FIXTURE_TEST_CASE(blockfilter_index_initial_sync, BuildChainTestingSetup)
{
    ...
    // Check that filter index has all blocks that were in the chain before it started.
    {
        ...
        for (block_index = m_node.chainman->ActiveChain().Genesis();
             block_index != nullptr;
             block_index = m_node.chainman->ActiveChain().Next(block_index)) {
            CheckFilterLookups(m_args.GetBlocksDirPath(), m_args.GetBoolArg("-fastprune", false), filter_index, block_index, last_header);
        }
    }
    ...
    for (size_t i = 0; i < 2; i++) {
        ...
        BOOST_CHECK(filter_index.BlockUntilSyncedToCurrentChain());
        CheckFilterLookups(m_args.GetBlocksDirPath(), m_args.GetBoolArg("-fastprune", false), filter_index, block_index, chainA_last_header);
    }
    ...
    for (size_t i = 0; i < 3; i++) {
        ...
        BOOST_CHECK(filter_index.BlockUntilSyncedToCurrentChain());
        CheckFilterLookups(m_args.GetBlocksDirPath(), m_args.GetBoolArg("-fastprune", false), filter_index, block_index, chainB_last_header);
    }
