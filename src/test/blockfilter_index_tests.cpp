...
static bool CheckFilterLookups(BlockFilterIndex& filter_index, const CBlockIndex* block_index,
                               uint256& last_header, BlockManager& blockman)
{
...
BOOST_FIXTURE_TEST_CASE(blockfilter_index_initial_sync, BuildChainTestingSetup)
{
    ...
    BlockFilterIndex filter_index(BlockFilterType::BASIC, 1 << 20, opts);
    ...
    // Check that filter index has all blocks that were in the chain before it started.
    {
        ...
        for (block_index = m_node.chainman->ActiveChain().Genesis();
             block_index != nullptr;
             block_index = m_node.chainman->ActiveChain().Next(block_index)) {
            CheckFilterLookups(filter_index, block_index, last_header, Assert(m_node.chainman)->m_blockman);
        }
    }
    ...
    for (size_t i = 0; i < 2; i++) {
        ...
        BOOST_CHECK(filter_index.BlockUntilSyncedToCurrentChain());
        CheckFilterLookups(filter_index, block_index, chainA_last_header, Assert(m_node.chainman)->m_blockman);
    }
    ...
    for (size_t i = 0; i < 3; i++) {
        ...
        BOOST_CHECK(filter_index.BlockUntilSyncedToCurrentChain());
        CheckFilterLookups(filter_index, block_index, chainB_last_header, Assert(m_node.chainman)->m_blockman);
    }
...
BOOST_FIXTURE_TEST_CASE(blockfilter_index_init_destroy, BasicTestingSetup)
{
    ...
    BOOST_CHECK(InitBlockFilterIndex(BlockFilterType::BASIC, 1 << 20, opts));
