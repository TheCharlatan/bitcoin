...
BOOST_AUTO_TEST_CASE(util_ReadConfigStream)
{
    ...
    test_args.SelectConfigNetwork(kernel::chainname::MAIN);
    BOOST_CHECK(test_args.GetArg("-d", "xxx") == "e");
    BOOST_CHECK(test_args.GetArgs("-ccc").size() == 2);
    BOOST_CHECK(test_args.GetArg("-h", "xxx") == "0");
            ...
            ForEachNoDup(conf_actions, SET, SECTION_NEGATE, [&] {
                for (bool soft_set : {false, true}) {
                    for (bool force_set : {false, true}) {
                        for (const std::string_view section : {kernel::chainname::MAIN, kernel::chainname::TESTNET, kernel::chainname::SIGNET}) {
                            for (const std::string_view network : {kernel::chainname::MAIN, kernel::chainname::TESTNET, kernel::chainname::SIGNET}) {
                                for (bool net_specific : {false, true}) {
                                    fn(arg_actions, conf_actions, soft_set, force_set, std::string{section}, std::string{network}, net_specific);
                                }
                            }
                        }
                    }
