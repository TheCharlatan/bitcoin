...
BOOST_AUTO_TEST_CASE(existing_data_no_obfuscate)
{
    ...
    std::unique_ptr<CDBWrapper> dbw = std::make_unique<CDBWrapper>(ph, (1 << 10), opts);
    uint8_t key{'k'};
    uint256 in = InsecureRand256();
    uint256 res;
    ...
    CDBWrapper::Options oopts = {
        .in_memory = false,
        .wipe_existing = false,
        .obfuscate_data = true,
    };
    CDBWrapper odbw(ph, (1 << 10), oopts);
...
BOOST_AUTO_TEST_CASE(unicodepath)
{
    ...
    CDBWrapper dbw(ph, (1 << 20), CDBWrapper::Options{});
