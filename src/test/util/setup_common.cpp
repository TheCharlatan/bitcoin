...
void ChainTestingSetup::LoadVerifyActivateChainstate()
{
    ...
    auto res = chainman.ActiveChainstate().ActivateBestChain(state);
    if (!res || !res.value()) {
    ...
    : ChainTestingSetup(chainType, extra_args)
{
    ...
    m_node.peerman = PeerManager::make(*m_node.connman, *m_node.addrman,
                                       ...
                                       m_node.exit_status, *m_node.shutdown,
                                                                     ...
                                                                     bool submit)
{
    ...
    if (submit) {
        LOCK(cs_main);
        const MempoolAcceptResult result = Assert(m_node.chainman->ProcessTransaction(MakeTransactionRef(mempool_txn))).value();
