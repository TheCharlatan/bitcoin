...
{
    return FlatFileSeq(gArgs.GetBlocksDirPath(), "blk", m_options.fast_prune ? 0x4000 /* 16kb */ : BLOCKFILE_CHUNK_SIZE);
}

FlatFileSeq BlockManager::UndoFileSeq() const
{
    return FlatFileSeq(m_options.blocks_dir, "rev", UNDOFILE_CHUNK_SIZE);
}
