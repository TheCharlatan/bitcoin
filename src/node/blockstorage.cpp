...
void BlockManager::FlushUndoFile(int block_file, bool finalize)
{
    ...
    if (!UndoFileSeq().Flush(undo_pos_old, finalize)) {
        AbortNode("Flushing undo file to disk failed. This is likely the result of an I/O error.", m_init_error_cb);
    }
}
...
void BlockManager::FlushBlockFile(bool fFinalize, bool finalize_undo)
{
    ...
    if (!BlockFileSeq().Flush(block_pos_old, fFinalize)) {
        AbortNode("Flushing block file to disk failed. This is likely the result of an I/O error.", m_init_error_cb);
    }
...
bool BlockManager::FindBlockPos(FlatFilePos& pos, unsigned int nAddSize, unsigned int nHeight, CChain& active_chain, uint64_t nTime, bool fKnown)
{
    ...
    if (!fKnown) {
        ...
        if (out_of_space) {
            return AbortNode("Disk space is too low!", m_init_error_cb, _("Disk space is too low!"));
        }
...
bool BlockManager::FindUndoPos(BlockValidationState& state, int nFile, FlatFilePos& pos, unsigned int nAddSize)
{
    ...
    if (out_of_space) {
        return AbortNode(state, "Disk space is too low!", m_init_error_cb, _("Disk space is too low!"));
    }
    if (bytes_allocated != 0 && IsPruneMode()) {
        m_check_for_pruning = true;
    }
...
bool BlockManager::WriteUndoDataForBlock(const CBlockUndo& blockundo, BlockValidationState& state, CBlockIndex& block)
{
    ...
    if (block.GetUndoPos().IsNull()) {
        ...
        if (!UndoWriteToDisk(blockundo, _pos, block.pprev->GetBlockHash(), GetParams().MessageStart())) {
            return AbortNode(state, "Failed to write undo data", m_init_error_cb);
        }
...
FlatFilePos BlockManager::SaveBlockToDisk(const CBlock& block, int nHeight, CChain& active_chain, const FlatFilePos* dbp)
{
    ...
    if (!position_known) {
        if (!WriteBlockToDisk(block, blockPos, GetParams().MessageStart())) {
            AbortNode("Failed to write block", m_init_error_cb);
            return FlatFilePos();
        }
    }
    return blockPos;
