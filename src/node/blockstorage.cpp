...
std::unique_ptr<BlockTreeStore> BlockManager::CreateAndMigrateBlockTree()
{
    ...
    if (!fs::exists(m_opts.block_tree_dir / "CURRENT")) {
        return std::make_unique<BlockTreeStore>(m_opts.block_tree_dir, m_opts.chainparams, m_opts.wipe_block_tree_data);
    }
...

    {
        ...
        auto block_tree_store{std::make_unique<BlockTreeStore>(migration_dir, m_opts.chainparams, m_opts.wipe_block_tree_data)};
