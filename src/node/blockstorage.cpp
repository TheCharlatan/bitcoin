...
static FlatFileSeq BlockFileSeq(const fs::path& blocks_dir, const bool fast_prune);
...
void BlockManager::CleanupBlockRevFiles() const
{
    ...
    for (fs::directory_iterator it(m_options.blocks_dir); it != fs::directory_iterator(); it++) {
...
bool BlockManager::UndoReadFromDisk(CBlockUndo& blockundo, const CBlockIndex* pindex) const
{
    ...
    AutoFile filein{OpenUndoFile(m_options.blocks_dir, pos, true)};
    if (filein.IsNull()) {
        return error("%s: OpenUndoFile failed", __func__);
    }
...
void BlockManager::FlushUndoFile(int block_file, bool finalize)
{
    ...
    if (!UndoFileSeq(m_options.blocks_dir).Flush(undo_pos_old, finalize)) {
...
void BlockManager::FlushBlockFile(bool fFinalize, bool finalize_undo)
{
    ...
    if (!BlockFileSeq(m_options.blocks_dir, m_options.fast_prune).Flush(block_pos_old, fFinalize)) {
...
void BlockManager::UnlinkPrunedFiles(const std::set<int>& setFilesToPrune) const
{
    ...
    for (std::set<int>::iterator it = setFilesToPrune.begin(); it != setFilesToPrune.end(); ++it) {
        FlatFilePos pos(*it, 0);
        const bool removed_blockfile{fs::remove(BlockFileSeq(m_options.blocks_dir, m_options.fast_prune).FileName(pos), ec)};
        const bool removed_undofile{fs::remove(UndoFileSeq(m_options.blocks_dir).FileName(pos), ec)};
...
bool BlockManager::FindBlockPos(FlatFilePos& pos, unsigned int nAddSize, unsigned int nHeight, CChain& active_chain, uint64_t nTime, bool fKnown)
{
    ...
    if (!fKnown) {
        bool out_of_space;
        size_t bytes_allocated = BlockFileSeq(m_options.blocks_dir, m_options.fast_prune).Allocate(pos, nAddSize, out_of_space);
        if (out_of_space) {
...
bool BlockManager::FindUndoPos(BlockValidationState& state, int nFile, FlatFilePos& pos, unsigned int nAddSize)
{
    ...
    bool out_of_space;
    size_t bytes_allocated = UndoFileSeq(m_options.blocks_dir).Allocate(pos, nAddSize, out_of_space);
    if (out_of_space) {
...
{
    // Open history file to append
    CAutoFile fileout(OpenBlockFile(blocks_dir, fast_prune, pos), SER_DISK, CLIENT_VERSION);
    if (fileout.IsNull()) {
...
bool BlockManager::WriteUndoDataForBlock(const CBlockUndo& blockundo, BlockValidationState& state, CBlockIndex* pindex, const CChainParams& chainparams)
{
    ...
    if (pindex->GetUndoPos().IsNull()) {
        ...
        if (!UndoWriteToDisk(m_options.blocks_dir, blockundo, _pos, pindex->pprev->GetBlockHash(), chainparams.MessageStart())) {
...
FlatFilePos BlockManager::SaveBlockToDisk(const CBlock& block, int nHeight, CChain& active_chain, const CChainParams& chainparams, const FlatFilePos* dbp)
{
    ...
    if (!position_known) {
        if (!WriteBlockToDisk(m_options.blocks_dir, m_options.fast_prune, block, blockPos, chainparams.MessageStart())) {
...
void ThreadImport(ChainstateManager& chainman, std::vector<fs::path> vImportFiles, const fs::path& mempool_path)
{
...

    {
        ...
        if (chainman.m_blockman.m_options.stop_after_block_import) {
