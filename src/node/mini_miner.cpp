...
MiniMinerMempoolEntry::MiniMinerMempoolEntry(txiter& entry) :
    fee_individual{entry.impl->GetModifiedFee()},
    tx{entry.impl->GetSharedTx()},
    vsize_individual(entry.impl->GetTxSize()),
    fee_with_ancestors{entry.impl->GetModFeesWithAncestors()},
    vsize_with_ancestors(entry.impl->GetSizeWithAncestors())
{ }
...
MiniMiner::MiniMiner(const CTxMemPool& mempool, const std::vector<COutPoint>& outpoints)
{
    ...
    for (auto& txiter : *cluster) {
        if (!m_to_be_replaced.count(txiter.impl->GetTx().GetHash())) {
            auto [mapiter, success] = m_entries_by_txid.emplace(txiter.impl->GetTx().GetHash(), MiniMinerMempoolEntry(txiter));
