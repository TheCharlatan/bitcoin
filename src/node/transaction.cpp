...

    {
        ...
        if (auto mempool_tx = node.mempool->get(txid); mempool_tx) {
            ...
            if (max_tx_fee > 0) {
                ...
                const MempoolAcceptResult result = Assert(node.chainman->ProcessTransaction(tx, /*test_accept=*/ true)).value();
            ...
            const MempoolAcceptResult result = Assert(node.chainman->ProcessTransaction(tx, /*test_accept=*/ false)).value();
            if (result.m_result_type != MempoolAcceptResult::ResultType::VALID) {
                return HandleATMPError(result.m_state, err_string);
            }
