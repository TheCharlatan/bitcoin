...
CTransactionRef GetTransaction(const CBlockIndex* const block_index, const CTxMemPool* const mempool, const uint256& hash, const Consensus::Params& consensusParams, uint256& hashBlock, const fs::path& blocks_dir, const bool fast_prune)
{
    ...
    if (block_index) {
        CBlock block;
        if (ReadBlockFromDisk(blocks_dir, fast_prune, block, block_index, consensusParams)) {
