...
bool FillBlock(const CBlockIndex* index, const FoundBlock& block, UniqueLock<RecursiveMutex>& lock, const CChain& active, const fs::path& blocks_dir)
{
    ...
    if (block.m_next_block) FillBlock(active[index->nHeight] == index ? active[index->nHeight + 1] : nullptr, *block.m_next_block, lock, active, blocks_dir);
    if (block.m_data) {
        REVERSE_LOCK(lock);
        if (!ReadBlockFromDisk(blocks_dir, *block.m_data, index, Params().GetConsensus())) block.m_data->SetNull();
    }
    block.found = true;
    return true;
}
...
public:
    ...
    bool findBlock(const uint256& hash, const FoundBlock& block) override
    {
        ...
        return FillBlock(chainman().m_blockman.LookupBlockIndex(hash), block, lock, chainman().ActiveChain(), chainman().BlocksDirPath());
    }
    bool findFirstBlockWithTimeAndHeight(int64_t min_time, int min_height, const FoundBlock& block) override
    {
        WAIT_LOCK(cs_main, lock);
        const CChain& active = chainman().ActiveChain();
        return FillBlock(active.FindEarliestAtLeast(min_time, min_height), block, lock, active, chainman().BlocksDirPath());
    }
    bool findAncestorByHeight(const uint256& block_hash, int ancestor_height, const FoundBlock& ancestor_out) override
    {
        ...
        if (const CBlockIndex* block = chainman().m_blockman.LookupBlockIndex(block_hash)) {
            if (const CBlockIndex* ancestor = block->GetAncestor(ancestor_height)) {
                return FillBlock(ancestor, ancestor_out, lock, active, chainman().BlocksDirPath());
            }
        }
        return FillBlock(nullptr, ancestor_out, lock, active, chainman().BlocksDirPath());
    }
    bool findAncestorByHash(const uint256& block_hash, const uint256& ancestor_hash, const FoundBlock& ancestor_out) override
    {
        ...
        return FillBlock(ancestor, ancestor_out, lock, chainman().ActiveChain(), chainman().BlocksDirPath());
    }
    bool findCommonAncestor(const uint256& block_hash1, const uint256& block_hash2, const FoundBlock& ancestor_out, const FoundBlock& block1_out, const FoundBlock& block2_out) override
    {
        ...
        // compiler warnings.
        return int{FillBlock(ancestor, ancestor_out, lock, active, chainman().BlocksDirPath())} &
               int{FillBlock(block1, block1_out, lock, active, chainman().BlocksDirPath())} &
               int{FillBlock(block2, block2_out, lock, active, chainman().BlocksDirPath())};
    }
