...
public:
    ...
    bool baseInitialize() override
    {
        ...
        m_context->validation_signals = std::make_unique<ValidationSignals>(std::make_unique<SerialTaskRunner>(*m_context->scheduler));
...
public:
    ...
    {
        return std::make_unique<NotificationsHandlerImpl>(chainman().m_signals, std::move(notifications));
    }
    void waitForNotificationsIfTipChanged(const uint256& old_tip) override
    {
        if (!old_tip.IsNull() && old_tip == WITH_LOCK(::cs_main, return chainman().ActiveChain().Tip()->GetBlockHash())) return;
        chainman().m_signals.SyncWithValidationInterfaceQueue();
    }
