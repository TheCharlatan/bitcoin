...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    if (m_options.test_block_validity) {
        auto res{TestBlockValidity(state, chainparams, m_chainstate, *pblock, pindexPrev,
                                   GetAdjustedTime, /*fCheckPOW=*/false, /*fCheckMerkleRoot=*/false)};
        if (!res) {
            auto error = ErrorString(res).original;
            AbortNode(error);
            throw std::runtime_error(strprintf("%s: TestBlockValidity failed with fatal error: %s", __func__, error));
        }
        if (!res.value()) {
