    ...
    const ChainstateLoadOptions& options) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    auto load_genesis_block_res{chainman.ActiveChainstate().LoadGenesisBlock()};
    if (!fReindex && (!load_genesis_block_res || !load_genesis_block_res.value())) {
                                                       ...
                                                       const ChainstateLoadOptions& options)
{
    ...
    if (IsFatal(snapshot_completion)) {
        return {util::Error{}, util::MoveMessages(snapshot_completion), ChainstateLoadError::FAILURE_FATAL};
    }

    if (snapshot_completion.value() == SnapshotCompletionResult::SKIPPED) {
        // do nothing; expected case
    } else if (snapshot_completion.value() == SnapshotCompletionResult::SUCCESS) {
...
util::Result<void, ChainstateLoadError> VerifyLoadedChainstate(ChainstateManager& chainman, const ChainstateLoadOptions& options)
{
    ...
    for (Chainstate* chainstate : chainman.GetAll()) {
        if (!is_coinsview_empty(chainstate)) {
            ...
            if (!res) {
                return {util::Error{ErrorString(res)}, ChainstateLoadError::FAILURE_FATAL};
            }
