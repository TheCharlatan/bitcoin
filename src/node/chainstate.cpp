                                    ...
                                    const ChainstateLoadOptions& options)
{
    ...
    auto snapshot_completion = chainman.MaybeCompleteSnapshotValidation(
        [&init_error_callback = chainman.m_options.init_error_callback](bilingual_str msg) {
    ...
    if (snapshot_completion == SnapshotCompletionResult::SKIPPED) {
        ...
        if (!chainman.ValidatedSnapshotCleanup()) {
            AbortNode("Background chainstate cleanup failed unexpectedly.", chainman.m_options.init_error_callback);
        }
...
ChainstateLoadResult VerifyLoadedChainstate(ChainstateManager& chainman, const ChainstateLoadOptions& options)
{
    ...
    for (Chainstate* chainstate : chainman.GetAll()) {
        if (!is_coinsview_empty(chainstate)) {
            ...
            VerifyDBResult result = CVerifyDB(chainman.m_options.show_progress_callback).VerifyDB(*chainstate, chainman.GetConsensus(), chainstate->CoinsDB(), options.check_level, options.check_blocks);
            switch (result) {
