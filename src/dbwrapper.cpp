 ...
 */
CDBBatch::CDBBatch(const CDBWrapper& _parent) : parent(_parent), m_pimpl_batch{std::make_unique<CDBBatch::WriteBatchImpl>()}, ssValue(SER_DISK, CLIENT_VERSION){};

CDBBatch::~CDBBatch() = default;

void CDBBatch::Clear()
{
    m_pimpl_batch->batch.Clear();
    size_estimate = 0;
}
...
void CDBBatch::WriteImpl(DataStream& ssKey, CDataStream& ssValue)
{
    ...
    m_pimpl_batch->batch.Put(slKey, slValue);
...
{
    leveldb::Slice slKey(CharCast(ssKey.data()), ssKey.size());
    m_pimpl_batch->batch.Delete(slKey);
...
{
    leveldb::Slice slKey(CharCast(ssKey.data()), ssKey.size());
    m_pimpl_iter->iter->Seek(slKey);
}

Span<const std::byte> CDBIterator::GetKeyImpl() const
{
    return MakeByteSpan(m_pimpl_iter->iter->key());
}

Span<const std::byte> CDBIterator::GetValueImpl() const
{
    return MakeByteSpan(m_pimpl_iter->iter->value());
}

CDBIterator::~CDBIterator() = default;
bool CDBIterator::Valid() const { return m_pimpl_iter->iter->Valid(); }
void CDBIterator::SeekToFirst() { m_pimpl_iter->iter->SeekToFirst(); }
void CDBIterator::Next() { m_pimpl_iter->iter->Next(); }
