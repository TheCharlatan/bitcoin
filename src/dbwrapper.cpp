...
bool CDBWrapper::ExistsImpl(const Span<const std::byte>& key) const
{
    leveldb::Slice slKey(CharCast(key.data()), key.size());
...
size_t CDBWrapper::EstimateSizeImpl(const Span<const std::byte>& key1, const Span<const std::byte>& key2) const
{
...
void CDBIterator::SeekImpl(const Span<const std::byte>& key)
{
    leveldb::Slice slKey(CharCast(key.data()), key.size());
