...
void CDBBatch::WriteImpl(const DataStream& ssKey, CDataStream& ssValue)
{
...
void CDBBatch::EraseImpl(const DataStream& ssKey)
{
    leveldb::Slice slKey(CharCast(ssKey.data()), ssKey.size());
...
bool CDBWrapper::ExistsImpl(const DataStream& ssKey) const
{
    leveldb::Slice slKey(CharCast(ssKey.data()), ssKey.size());
