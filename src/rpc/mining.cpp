        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
...

    {
        ...
        if (!TestBlockValidity(state, chainparams, chainman.ActiveChainstate(), block, chainman.m_blockman.LookupBlockIndex(block.hashPrevBlock), GetAdjustedTime, false, false)) {
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!request.params[0].isNull())
    {
        ...
        if (strMode == "proposal")
        {
            ...
            BlockValidationState state;
            TestBlockValidity(state, Params(), active_chainstate, block, pindexPrev, GetAdjustedTime, false, true);
