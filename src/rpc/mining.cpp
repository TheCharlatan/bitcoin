        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
...

    {
        ...
        if (!CheckFatal(TestBlockValidity(state, chainman.GetParams(), chainman.ActiveChainstate(), block, chainman.m_blockman.LookupBlockIndex(block.hashPrevBlock), false, false),
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!request.params[0].isNull())
    {
        ...
        if (strMode == "proposal")
        {
            ...
            BlockValidationState state;
            (void)CheckFatal(TestBlockValidity(state, chainman.GetParams(), active_chainstate, block, pindexPrev, false, true), node.shutdown, node.exit_status);
