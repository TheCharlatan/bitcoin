...
static UniValue generateBlocks(ChainstateManager& chainman, const CTxMemPool& mempool, const CScript& coinbase_script, int nGenerate, uint64_t nMaxTries, NodeContext& node)
{
    ...
    while (nGenerate > 0 && !chainman.m_interrupt) {
        std::unique_ptr<CBlockTemplate> pblocktemplate(BlockAssembler{chainman.ActiveChainstate(), &mempool, *node.shutdown, node.exit_status}.CreateNewBlock(coinbase_script));
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    ChainstateManager& chainman = EnsureChainman(node);
    {
        ...
        std::unique_ptr<CBlockTemplate> blocktemplate(BlockAssembler{chainman.ActiveChainstate(), nullptr, *node.shutdown, node.exit_status}.CreateNewBlock(coinbase_script));
        if (!blocktemplate) {
            throw JSONRPCError(RPC_INTERNAL_ERROR, "Couldn't create new block");
        }
...

    {
        ...
        if (!CheckFatal(TestBlockValidity(state, chainman.GetParams(), chainman.ActiveChainstate(), block, chainman.m_blockman.LookupBlockIndex(block.hashPrevBlock), GetAdjustedTime, false, false),
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!request.params[0].isNull())
    {
        ...
        if (strMode == "proposal")
        {
            ...
            BlockValidationState state;
            (void)CheckFatal(TestBlockValidity(state, chainman.GetParams(), active_chainstate, block, pindexPrev, GetAdjustedTime, false, true), node.shutdown, node.exit_status);
        ...
        (mempool.GetTransactionsUpdated() != nTransactionsUpdatedLast && GetTime() - time_start > 5))
    {
        ...
        pblocktemplate = BlockAssembler{active_chainstate, &mempool, *node.shutdown, node.exit_status}.CreateNewBlock(scriptDummy);
        if (!pblocktemplate)
