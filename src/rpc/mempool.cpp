    ...
    return RPCHelpMan{"testmempoolaccept",
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            const PackageMempoolAcceptResult package_result = [&] {
                LOCK(::cs_main);
                if (txns.size() > 1) {
                    auto res{HandleFatalError(ProcessNewPackage(chainstate, mempool, txns, /*test_accept=*/true), node.shutdown, node.exit_status)};
...
static RPCHelpMan submitpackage()
{
    return RPCHelpMan{"submitpackage",
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            const auto res = WITH_LOCK(::cs_main, return HandleFatalError(ProcessNewPackage(chainstate, mempool, txns, /*test_accept=*/ false), node.shutdown, node.exit_status));
            if (!res) throw JSONRPCError(RPC_INTERNAL_ERROR, "Fatal internal error.");
