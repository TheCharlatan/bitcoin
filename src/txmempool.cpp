...
CTxMemPool::Entries CTxMemPool::CalculateAncestors(const Entries& parents) const
{
    ...
    while (!work_queue.empty()) {
        ...
        for (auto parent : it.get().GetMemPoolParentsConst()) {
            if (!visited(parent)) {
...
CTxMemPool::setEntryRefs CTxMemPool::CalculateMemPoolAncestorsSlow(
...
void CTxMemPool::CalculateDescendantsSlow(const CTxMemPoolEntry& entry, setEntryRefs& setDescendants) const
{
    auto descendants = CalculateDescendants({entry});
...
CTxMemPool::Entries CTxMemPool::CalculateDescendants(Entries txs) const
{
    ...
    while (!work_queue.empty()) {
        const CTxMemPoolEntry& it = work_queue.back();
        work_queue.pop_back();
        result.push_back(it);
