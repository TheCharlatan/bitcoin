...
CTxMemPool::CTxMemPool(const Options& opts, util::Error& error)
      ...
      m_signals{opts.signals}
{
    ...
    if (opts.max_size_bytes < 0 || opts.max_size_bytes < descendant_limit_bytes) {
        error = util::Error{strprintf(_("-maxmempool must be at least %d MB"), std::ceil(descendant_limit_bytes / 1'000'000.0))};
    }
}
