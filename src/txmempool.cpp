...
static std::vector<CTxMemPool::indexed_transaction_set::const_iterator> GetSortedDepthAndScore(
    const CTxMemPool::indexed_transaction_set& mapTx, RecursiveMutex& cs) EXCLUSIVE_LOCKS_REQUIRED(cs)
{
    std::vector<CTxMemPool::indexed_transaction_set::const_iterator> iters;
    AssertLockHeld(cs);

    iters.reserve(mapTx.size());

    for (CTxMemPool::indexed_transaction_set::iterator mi = mapTx.begin(); mi != mapTx.end(); ++mi) {
        iters.push_back(mi);
    }
