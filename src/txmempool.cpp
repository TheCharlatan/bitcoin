    ...
    const CTxMemPool::Limits& m_limits)
{
    ...
    while (!stageEntries.empty()) {
        ...
        for (const CTxMemPoolEntry& childEntry : children) {
            ...
            if (cacheIt != cachedDescendants.end()) {
                ...
                for (raw_txiter cacheEntry : cacheIt->second.impl) {
    ...
    for (const CTxMemPoolEntry& descendant : descendants) {
        if (!setExclude.count(descendant.GetTx().GetHash())) {
            ...
            modifyCount++;
            cachedDescendants[updateIt.impl].impl.insert(mapTx->impl.iterator_to(descendant));
    ...
    mapTx->impl.modify(updateIt.impl, [=](CTxMemPoolEntry& e) { e.UpdateDescendantState(modifySize, modifyFee, modifyCount); });
}
...
static std::vector<MemPoolMultiIndex::indexed_transaction_set::const_iterator> GetSortedDepthAndScore(
    const std::unique_ptr<MemPoolMultiIndex::MapTxImpl>& mapTx, RecursiveMutex& cs
