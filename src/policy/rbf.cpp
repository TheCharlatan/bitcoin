...
RBFTransactionState IsRBFOptIn(const CTransaction& tx, const CTxMemPool& pool)
{
    ...
    const CTxMemPoolEntry& entry_for_ancestors{*Assert(pool.GetEntry(tx.GetHash()))};
    auto ancestors{pool.CalculateMemPoolAncestors(entry_for_ancestors, /*fSearchForParents=*/false)};
                                                  ...
                                                  CTxMemPool::setEntryRefs& all_conflicts)
{
    ...
    for (const CTxMemPoolEntry& entry: entries_conflicting) {
        ...
        pool.CalculateDescendantsSlow(entry, all_conflicts);
    }
    if (all_conflicts.size() > MAX_REPLACEMENT_CANDIDATES) {
                                                 ...
                                                 const uint256& txid)
{
    for (const CTxMemPoolEntry& entry : entries_conflicting) {
        ...
        Cluster::Chunk &chunk = entry.m_cluster->m_chunks[entry.m_loc.first];
