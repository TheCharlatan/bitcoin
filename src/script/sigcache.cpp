...
void CSignatureCache::ComputeEntrySchnorr(uint256& entry, const uint256 &hash, Span<const unsigned char> sig, const XOnlyPubKey& pubkey) const
{
    CSHA256 hasher = m_salted_hasher_schnorr;
    hasher.Write(hash.begin(), 32).Write(pubkey.data(), pubkey.size()).Write(sig.data(), sig.size()).Finalize(entry.begin());
}

bool CSignatureCache::Get(const uint256& entry, const bool erase)
{
...
std::optional<std::pair<uint32_t, size_t>> CSignatureCache::setup_bytes(size_t n)
{
